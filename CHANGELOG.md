...
This release is being built in phases for the "Adaptive Model Router v1." Phases 0 through 3 now provide contracts, static profiling, Registry v2, a local benchmark, and the pure adaptive-router core. Runtime model-selection integration and router UI remain future phases — see `docs/roadmap.md`.
...
- Added `RouterInput`/`RouterDecision` in `@free-ai-open/model-router`: the adaptive core consumes task, locale, mode, coarse capability, optional benchmark, technical observations, cache IDs, registry version, and optional manual model ID. It returns nullable selection, fallbacks, confidence, stable codes, token budgets, rejected candidates, and score components. It coexists with the active v0.6 router, which remains the app's runtime path until Phase 4.
