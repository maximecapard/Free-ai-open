...
- **v0.8.0-alpha Phase 0 (contracts and architecture only -- no benchmark runner or `/benchmarks` page yet, see docs/roadmap.md).** Defined the versioned local data contract, persistence architecture, and metric semantics for user-visible MODEL performance benchmarking -- distinct from the existing generic device/WebGPU compute microbenchmark (`LocalBenchmarkResult`), which measures raw device throughput independent of any model.
  - Added `@free-ai-open/types`' `benchmark-signals.ts`: `ModelBenchmarkResult` and its sub-shapes (`ModelBenchmarkModelReference`, `ModelBenchmarkRunConfig`, `ModelBenchmarkLoadMeasurement`, `ModelBenchmarkFirstTokenMeasurement`, `ModelBenchmarkGenerationMeasurement`, `ModelBenchmarkEnvironment`) plus the `ModelBenchmarkPreset` ("quick"/"standard"/"extended") and `ModelBenchmarkContextPreset` vocabularies. `ModelBenchmarkOutcome` is deliberately the exact same type as `ModelPerformanceObservation["outcome"]`, so a benchmark run and a real chat generation share one terminal-outcome vocabulary by construction.
  - Defined exact metric semantics: load time (model-load request to runtime-ready, with `wasModelCachedBeforeRun`/`modelLoadedDuringRun` distinguishing a fresh download from a cached load from no load at all), time to first token (inference start to the earliest raw runtime token, independent of React/buffered rendering), and generation throughput (`tokenCountConfidence: "exact" | "unavailable"` -- a real runtime/tokenizer-backed count or nothing at all, never a character-length-derived tok/s).
  - Defined benchmark stability classification consistent with the existing adaptive router: `completed` is positive; `cancelled`/`length_limited`/`unsupported_tool_call`/`terminal_unknown` are neutral (a user-cancelled benchmark run must never penalize the model); `stalled`/`degenerate`/`out_of_memory`/`device_lost`/`load_failed` are negative. Computed by a pure function (`classifyModelBenchmarkStability()`), never persisted on the result itself, so a future refinement of the classification applies retroactively to already-stored history.
  - Added the new `@free-ai-open/model-benchmark` package: local IndexedDB-backed persistence (with an in-memory fallback for SSR/tests, mirroring `@free-ai-open/conversation-store`'s architecture) for `ModelBenchmarkResult` history, with a bounded history (100 results total, 20 per model, oldest dropped first, pruned atomically alongside each insert -- never a state where the new record was written but the cap was not yet enforced) and compatibility/invalidation helpers (`isModelBenchmarkResultExpired`, `isModelBenchmarkResultCompatible`, `isModelBenchmarkResultUsable`, `filterUsableModelBenchmarkResults`) that require an exact match on this package's own current `benchmarkVersion`, registry version, WebLLM version, coarse capability-profile key, AND browser family -- any single mismatch or expiry means the result is ignored outright, never down-weighted.
  - `sanitizeModelBenchmarkResult()` rebuilds an exact allowlisted shape from scratch on every write and read, and now enforces every invariant as actual runtime validation rather than convention alone: an `"exact"`-confidence generation measurement's token count, duration, and rate must all be present and numerically consistent with each other within a small tolerance; `capabilityProfileKey` must match the same low-entropy, four-segment grammar `buildCapabilityProfileKey()` produces (an arbitrary bounded string, a UUID, or an exact GPU description can no longer pass); `expiresAt` must equal `createdAt` plus the package's own TTL exactly (a forged far-future expiry is rejected); and stage/outcome/measurement combinations are checked against an explicit legality table (e.g. `not_started` can never pair with `completed`, and a failed load can never carry generation metrics). Every storage backend re-runs this same sanitizer on its own write path as defense in depth, and the raw backends are no longer exported from the package's public index -- `ModelBenchmarkStoreClient` is the only sanctioned way to persist a result.
  - Added `summarizeModelBenchmarkEvidence()` and `BenchmarkEvidenceSummary`: a pure aggregation contract (sample count, most-recent timestamp, median load/first-token/throughput, stable/neutral/negative run counts, and a count-gated `evidenceLevel` of `"none" | "weak" | "moderate" | "strong"`) that a future router-integration phase can consume without this phase deciding any router scoring weight. A single sample is always `"weak"`, never anything stronger, regardless of how good that one run looked.
  - Extended `apps/web/app/_lib/packageDependencyBoundaries.test.ts` (previously v0.7.0-alpha router/runtime/registry only) to also cover `@free-ai-open/model-benchmark`: depends on nothing but `@free-ai-open/types`, no dependency on `model-router`/`ai-runtime`/`model-registry`, and no cycle.
  - No benchmark runner, no `/benchmarks` page, and no router-scoring wiring yet -- see docs/architecture.md's "Local model benchmarking" section for the defined-but-not-yet-implemented relationship to the adaptive router.
  - Never persists benchmark prompt text, generated response content, exact GPU identifiers, or any unique fingerprinting identifier -- only bounded technical measurements and the same coarse references (`capabilityProfileKey`, `browserFamily`) already used elsewhere in this codebase. See docs/privacy.md's "Local model benchmarking" section.

### Changed

- Cancelled, failed, or degenerate assistant partial output is not saved as a normal completed assistant message. A genuine stall or absolute safety-limit interruption may preserve visible partial output, but it is durably marked `incomplete` in local conversation storage and in local export/import files.
...
## [0.7.1-alpha] - 2026-07-19 (generation timeout/stall watchdog hotfix, safe Markdown rendering, reasoning presentation and finish-reason handling)

A release-blocking bug report: a response could be actively streaming visible text and still get abruptly stopped by a false "generation timeout." This release fixes the watchdog so a timeout can only mean absence of runtime progress, never merely that a generation ran long while still producing output. It also adds safe Markdown rendering and copyable fenced code blocks for assistant responses, which previously displayed raw `**bold**`/list/fence syntax as plain text. `v0.7.0-alpha` itself is unchanged and its release notes above are not rewritten — both issues described here were present in that release.

A second, independently reported issue from a real exported conversation and diagnostic: a Qwen3 model's reply could persist ending abruptly inside an unclosed `<think>` reasoning block, with `inference.completed` recorded and no indication anything had gone wrong. Diagnosis (see `docs/DEVLOG.md`'s Sprint 6.21 "reasoning/finish-reason" subsections) found the generation had exhausted its output-token budget — WebLLM's own `finish_reason: "length"` — while `packages/ai-runtime/src/runtime.ts` only ever distinguished `"abort"` from everything else, silently folding a truncated reply into the same outcome as a normal completed one. This release adds an explicit `"length"` stop reason throughout the runtime/persistence/observation chain, a focused collapsible reasoning panel that never shows raw `<think>` markup, explicit interrupted-reasoning notices, and a bounded manual Continue action.

### Added

- Added safe Markdown rendering for assistant responses (`apps/web/app/_components/MessageContent.tsx`), replacing the previous plain-text/`white-space: pre-wrap` display: paragraphs, single-newline line breaks, bold/italic, ordered/unordered lists (including nesting), headings (capped well under the app's own page-title size), blockquotes, horizontal rules, links, inline code, fenced code blocks, and GFM tables (in a horizontally scrollable wrapper).
- Added `react-markdown`, `remark-gfm`, and `remark-breaks` as the Markdown dependency set (see "Dependencies" below) — no Markdown rendering package existed in the repo before this. Parsing happens only at the UI layer; the persisted/exported message content is always the original plain Markdown source string, byte for byte.
- Added `apps/web/app/_components/CodeBlock.tsx`: fenced code blocks render in a dedicated Ink-surface component (reusing the existing `.fo-ink-surface` dark tokens, not a new color system) with an optional language label derived from the fence (a neutral "Code" label when none is given, never invented), a "Copy"/"Copier" button with temporary "Copied"/"Copié" or "Could not copy"/"Impossible de copier" feedback, monospace text, preserved indentation, and internal horizontal scrolling for long lines instead of expanding the page.
- Added `apps/web/app/_lib/codeBlockCopy.ts` (pure Clipboard API wrapper, handles permission rejection/an unavailable clipboard/empty code) and `apps/web/app/_lib/markdownLinkSafety.ts` (a strict `http:`/`https:`/`mailto:` URL allowlist used for every Markdown link and, defensively, image source).
- Added `common.copy`/`common.copied`/`common.couldNotCopy` and `chat.codeBlockLabel` translation keys (English/French).
- Added a focused reasoning disclosure for Qwen3-style `<think>` blocks (`apps/web/app/_components/ReasoningDisclosure.tsx`): a native `<details>`/`<summary>` panel, expanded by default while reasoning is open (streaming or interrupted) and collapsing itself exactly once, automatically, the moment `</think>` arrives, without fighting a later manual toggle. Shows "Thinking…"/"Réflexion en cours…" while the message is the actively streaming generation and the static "Thinking"/"Réflexion" label once it is not. Reasoning content renders through the same safe Markdown renderer as the final answer (see "Message rendering" above).
- Added `apps/web/app/_lib/reasoningSegmentation.ts`: a pure, dependency-free string scan (not a general HTML/XML parser, since `<think>` is a single well-known, never-nested Qwen3 marker) that splits raw assistant content into an optional reasoning segment and the surrounding final-answer text, and reports whether a `<think>` block is still open. Presentation-only — it never mutates or re-saves persisted content, and each message is segmented independently of every other.
- Added an explicit interrupted-reasoning notice, shown only once a `<think>` block is open and the message is no longer the active generation: a generic "The generation stopped before the final answer."/"La génération s'est arrêtée avant la réponse finale." for every case, and a length-specific "The generation limit was reached."/"La limite de génération a été atteinte." shown only when this session itself knows the cause (a fresh generation's own WebLLM `finish_reason: "length"` — never inferred for a reloaded or imported historical conversation, which always falls back to the generic notice).
- Added a manual "Continue"/"Continuer" action for a message left incomplete (either an explicit `status: "incomplete"` or, for a message persisted before that status field existed, an unclosed `<think>` block with no generation active — see `apps/web/app/_lib/continuationEligibility.ts`). Continue extends the same message in place (`updateMessageContent`, a new `@free-ai-open/conversation-store` method — see "Changed" below) rather than creating a duplicate; the continuation prompt (`apps/web/app/_lib/continuationPrompt.ts`) restates the original request and the partial reply with an explicit "do not repeat" instruction, since the runtime only ever sees one prompt string per generation and does not read prior turns from conversation history. Bounded to 3 continuations per message (manual click, never an automatic/invisible retry loop); cancelling a continuation reverts to the pre-continuation content instead of discarding the whole reply.
...
- Hardened the desktop `/chat` viewport containment found during manual review: `.chat-history-panel` — `ChatHistoryDrawerPanel`'s outer wrapper and the actual flex child of `.chat-layout` on desktop, not `.chat-sidebar` two levels further down — had no explicit height/overflow containment of its own in the desktop workspace rules, and neither did `.chat-shell`/`.chat-layout` themselves; both relied entirely on flex cross-axis stretch behavior and a descendant's own scroll region to stay bounded. Made every level explicit (`height: 100%`/`min-height: 0`/`overflow: hidden` on `.chat-history-panel`, `.chat-shell`, and `.chat-layout`) and added an outermost `body:has(.chat-shell) { overflow: hidden }` safety net, scoped to the chat route only. Verified with up to 341 seeded conversations and long multi-code-block messages in both a dev and a production build; could not reproduce the originally reported symptom (document scrolling several viewport heights) under those conditions, but the previous implicit containment left no explicit guarantee at these levels, so this closes that gap defensively rather than leaving it to flexbox stretch semantics. Mobile is unaffected — every new rule lives inside the existing `@media (min-width: 721px)` block.
- Fixed a length-limited generation being silently recorded and presented as a normal successful completion. `packages/ai-runtime/src/runtime.ts` now captures WebLLM's actual `finish_reason` from the completion stream (`"stop" | "length" | "tool_calls" | "abort"`) instead of only ever checking for `"abort"`, and returns a new, explicit `GenerationStopReason` value, `"length"` (`packages/ai-runtime/src/types.ts`) — distinct from `"completed"`, `"cancelled"`, and `"degenerate_output"`. The runtime still returns to `ready`: WebLLM itself did not error, so a length-limited generation is never scored as a stall or model-instability failure (`apps/web/app/_lib/performanceObservationBuilder.ts` classifies it as its own neutral `"length_limited"` outcome, excluded from both success and instability scoring -- see "Additional post-review hardening" below, which replaced this bullet's original `completed` classification once the router-confidence implication of that was found), while `apps/web/app/_lib/generationPersistence.ts` still marks the message `status: "incomplete"` and preserves its partial output rather than discarding or silently accepting it as done.
- Fixed the real, back-calculated binding output-token limit behind the reported bug: with no strong performance evidence yet for an early-session generation, `@free-ai-open/model-router`'s adaptive preset selection falls back from the "performance" preset to "balanced" (512 output tokens) even when performance mode is selected (existing, unchanged behavior — see `hasStrongPerformanceEvidence()`); a second, previously undocumented global cap in `packages/ai-runtime/src/generation-safety.ts` (`GENERATION_SAFETY_LIMITS.maxTokens`, 768) then silently re-clipped even the full "performance" preset's budget for 3 of the 5 registry models. Raised to 2048 so the reasoning-aware budget below is not immediately re-clipped by this second ceiling.
...
- Kept a wholly separate, much more conservative emergency safety cap (10 minutes, `ABSOLUTE_GENERATION_SAFETY_LIMIT_MS`) to bound a truly pathological runaway generation. It uses its own distinct `generation_exceeded_safety_limit` error code — never `generation_stalled`, and the old `generation_timeout` code is retired entirely — and is classified as a neutral `cancelled` observation rather than model success, failure, or instability (see "Security and Privacy").
- A genuine stall or safety-limit interruption that already produced visible assistant text now preserves and locally saves that partial reply with durable `status: "incomplete"` provenance. The status survives refresh and local export/import and is shown in the transcript; an interruption with no output at all still discards the empty bubble. Stop and degenerate-output handling are unchanged.
- Forced watchdog recovery now emits the matching terminal runtime error when the abandoned worker stream eventually settles, ignores every late chunk before processing it, and keeps forced outcomes isolated by generation epoch. The app-level runtime-error path remains a fallback for worker streams that never settle.
- Conversation message appends and title updates now use atomic store updates, preventing an asynchronous append from recreating a conversation that was deleted concurrently. A failed partial-response write is surfaced through the existing local-storage notice.
...
- User-entered messages remain plain text with preserved line breaks — never parsed as Markdown. A pasted code snippet or a line a user typed starting with `*`/`#` should never be silently reformatted; this matches the least-surprising behavior of the chat products this mirrors. Only assistant responses go through the new Markdown renderer.
- The Markdown renderer never rerenders unaffected historical messages on each streamed chunk: `MessageContent` and each `ChatMessageBubble` stay memoized on their own message object, which already only changes reference for the one message actively streaming (`persistentGenerationState.ts`'s existing behavior, unchanged by this release).
- Added a narrow, reasoning-aware output-token budget: `packages/model-router/src/adaptiveOutputBudget.ts` adds a flat `+512` token allowance on top of the selected preset's `maxOutputTokens`, but only for candidates from a model family that reasons by default (currently `Qwen3` — the registry's `family` field already distinguishes this; `SmolLM2`/`Qwen2.5-Coder` are unaffected). Deliberately narrow in scope for this hotfix: no model-registry schema change, and no change to `adaptiveRouter.ts`'s preset/model eligibility or scoring — only the output-token budget number widens for the models that actually need reasoning headroom.
- Inspected WebLLM's own `enable_thinking` chat-completion option (it prepends `<think>\n\n</think>\n\n` to suppress reasoning tokens entirely when set to `false`). Documented in `docs/architecture.md`'s "Reasoning output and finish reason" section as a real, available lever for a future release, but deliberately not wired up in this hotfix — this release's scope is presenting and budgeting for reasoning output correctly, not deciding whether to request it at all.
- Old, already-persisted conversations containing raw `<think>...</think>` markup (including one with an unclosed `<think>` from before this feature existed) render through the same reasoning disclosure and interrupted-detection logic with no migration step and no rewrite of stored content — `apps/web/app/_lib/continuationEligibility.ts` recognizes both an explicit `status: "incomplete"` and a still-open `<think>` block with no status field at all, so the Continue action and the interrupted notice both work identically on historical data.
...
- The absolute safety-limit cap is classified as neutral `cancelled`, not `completed`, `failed`, or `stalled`, in local `ModelPerformanceObservation`s (`apps/web/app/_lib/performanceObservationBuilder.ts`). It is therefore excluded from success/failure scoring and cannot improve or penalize a model merely because the app enforced its emergency duration cap.
...
- The Markdown renderer never enables raw HTML: no `rehype-raw`/`rehype-sanitize` plugin is used, so a `<script>`, an `onerror=` attribute, or a `<style>` block found in model output is dropped during parsing rather than rendered, and `dangerouslySetInnerHTML` is never used for model output anywhere in the app. Every link and image URL is passed through a strict `http:`/`https:`/`mailto:` allowlist (`markdownLinkSafety.ts`); anything else, including `javascript:`/`data:` schemes and bare relative paths, resolves to plain non-clickable text. External links always get `target="_blank" rel="noopener noreferrer nofollow"`.
- Markdown images are not rendered as `<img>` at all — displaying one would make the browser fetch a model-chosen URL with zero user confirmation, which this local-first app never wants to trigger implicitly. The alt text/URL is shown as safe plain text (or a sanitized link) instead.
- The code-copy button copies text to the clipboard only; the copied/rendered code is never written to `logEvent`, local technical logs, or diagnostic reports — confirmed by a source-scanning test, not only by omission.
- Markdown source stays byte-for-byte unchanged in local storage and export/import: rendering happens only in `MessageContent.tsx` at display time, and no persistence/export code path was touched by this change.
- Reasoning content is never written to `logEvent`, local technical logs, or diagnostic reports — confirmed by a source-scanning test (`ReasoningDisclosure.test.tsx`), the same pattern used for the Markdown renderer above, not only by omission. The Continue action's composed continuation prompt (original request plus partial reply plus a "do not repeat" instruction) is sent to the model only; it is never itself persisted as a message, exported, or logged — only the model's own streamed output, appended to the existing assistant message, is saved.
- Revised after an initial round of external review: the interrupted-reasoning reason is now durably persisted as `ConversationMessage.incompleteReason` (`@free-ai-open/conversation-store`), round-tripped through local export/import, and still never written to local logs or diagnostic reports (only where the reason lives changed, not the no-content-in-logs guarantee). A length-limited reply now shows its specific notice after a reload, not only within the session that generated it; a message from before this field existed still falls back correctly to the generic notice. See "Additional post-review hardening" below.
- `packages/diagnostic-report`'s existing content-exclusion tests (`index.test.ts`) already cover this release without a code change: they assert no message-content field, including one carrying arbitrary text, ever appears in an exported diagnostic report, which equally covers reasoning content — reviewed and confirmed rather than re-implemented.

### Dependencies

- Added `react-markdown` (^10.1.0), `remark-gfm` (^4.0.1), and `remark-breaks` (^4.0.0) to `apps/web`. No Markdown rendering or syntax-highlighting package previously existed in the repository. `react-markdown` was chosen specifically because it renders a Markdown AST directly to React elements — never an HTML string — so there is no `dangerouslySetInnerHTML` trust boundary to defend at all, unlike an HTML-string-based renderer (`marked`/`markdown-it`) would require. No syntax-highlighting dependency was added; fenced code blocks render as clean, readable, unhighlighted monospace text, per this release's own scope (a large client-side highlighter is left for a later release).
- Bundle effect: the react-markdown/remark/micromark stack adds one route-scoped chunk of ~174 KB uncompressed (~52 KB gzipped) to the production build, loaded only by `/chat` (the sole route importing `MessageContent`) — it is not part of the initial bundle for `/`, `/settings`, `/debug`, or onboarding.
...
- Extended `packages/ai-runtime/src/runtime.test.ts` with a regression test proving a generation streaming continuously past the old absolute-duration threshold completes successfully (this test fails against the pre-fix implementation with a false `generation_timeout`), plus coverage for a genuine mid-stream stall, late non-empty chunk rejection, forced terminal errors, a slow downstream UI buffer never affecting the watchdog, stale-generation isolation, Stop/completion/runtime-recovery clearing every timer, the new absolute safety limit and its distinct error code, watchdog suspend/resume, and privacy of the new log events.
- Extended generation-persistence and performance-observation tests, added IndexedDB delete/append race coverage, verified durable incomplete-message refresh mapping, and verified that local conversation export/import validates and preserves message completion status.
- Added `apps/web/app/_components/MessageContent.test.tsx`, rendering to a static HTML string (`react-dom/server`, no new jsdom/testing-library dependency, since this repo's default test environment has no DOM): paragraph/line-break/bold/italic/list/heading/blockquote/hr/table rendering, inline code vs. fenced code-block distinction, a labeled Python fence, a fence without a language, multiple independent code blocks in one message, raw `<script>`/event-handler/`<style>` HTML never rendering, `javascript:` links neutralized to plain text, safe external-link attributes, images never becoming `<img>` elements, an unclosed streamed fence not crashing or losing text, no content duplication once a streamed fence closes, one message's unclosed fence never bleeding into another message, the internally-scrollable code-block class always being applied, and a source-scan guard proving the renderer/copy path never calls a logger.
- Added `apps/web/app/_lib/codeBlockCopy.test.ts` (exact-text copy, clipboard rejection, unavailable clipboard, empty code) and `apps/web/app/_lib/markdownLinkSafety.test.ts` (allowed schemes, `javascript:`/`data:`/`vbscript:`/`file:` rejection, relative/protocol-relative rejection, empty input).
- Extended `packages/ai-runtime/src/runtime.test.ts` with the natural-stop-vs-length distinction, `"length"` classification, and a regression fixture reproducing the real BeautifulSoup-request bug (a generation whose entire output budget is spent inside an unclosed `<think>` block).
- Added `apps/web/app/_lib/reasoningSegmentation.test.ts` (no-`<think>`-tag content, a complete block, an unclosed block, text before an opening tag, malformed/stray closing tags, an empty string, multiple `<think>` tags, and the `isReasoningInterrupted` streaming/interrupted/closed/never-reasoned matrix) and `apps/web/app/_lib/continuationEligibility.test.ts` (explicit `incomplete` status, explicit `complete` status, an old unclosed-`<think>`-with-no-status message, an old closed-reasoning message with no status, and a plain old message with neither).
- Added `apps/web/app/_components/ReasoningDisclosure.test.tsx` and `apps/web/app/_components/ChatTranscript.test.tsx`, rendered statically via `react-dom/server` (same approach as `MessageContent.test.tsx` — this repo's default test environment has no DOM): the in-progress vs. static label, the generic vs. length-specific interrupted notice (and that closing reasoning suppresses both), reasoning rendered through the same safe Markdown renderer, a user message's literal `<think>` text never treated as reasoning markup, the generic incomplete label suppressed when the reasoning notice already explains the interruption (and still shown for a plain non-reasoning incomplete reply), a real historical raw-`<think>` conversation (old, closed reasoning, no `status` field) rendering correctly with no migration, and a source-scanning guard that reasoning rendering never calls a logger.
- Added `apps/web/app/_lib/continuationPrompt.test.ts` (the original request and partial content both present, English and French "do not repeat/restart" instructions) and extended `packages/conversation-store/src/index.test.ts` with the new `updateMessageContent` (in-place replacement; no-op for an unknown message ID).
- Extended `packages/model-router/src/adaptiveOutputBudget.test.ts` (new) and `adaptiveRouter.test.ts` for the reasoning-family output-token allowance, and `apps/web/app/_lib/generationPersistence.test.ts` / `performanceObservationBuilder.test.ts` for the `"length"` reason's discard/notice/classification rules, including an explicit natural-stop-shows-no-warning case.
- Manually verified end-to-end against the real reported conversation and diagnostic export (`freeai-open-conversation-*.json`/`freeai-open-diagnostic-*.json`, imported through the app's own Import action, not a synthetic fixture): the historical interrupted message renders with the generic notice and a Continue button with no code changes needed for old data; clicking Continue extends the same message bubble (confirmed via the "Thinking…" in-progress label reappearing on the existing bubble, not a new one) using the real local WebLLM runtime; and cancelling mid-continuation reverts to the pre-continuation content with the existing "stopped, not saved" notice and no duplicate message, all with zero console errors.
- Extended `apps/web/app/_lib/conversationMessages.test.ts` to prove Markdown source content survives the conversation-to-transcript mapping unchanged, and `apps/web/app/_i18n/i18n.test.ts` with the exact French/English copy-feedback strings.
- Extended `apps/web/app/_lib/chatShellLayout.test.ts` (a structural test that reads `globals.css` directly) with assertions that `.chat-history-panel`, `.chat-shell`, and `.chat-layout` each carry explicit desktop containment, and that the new `body:has(.chat-shell)` safety net exists without the plain unscoped `body` rule ever gaining `overflow: hidden`.
- Confirmed the full existing generation-timeout, Stop, and recovery test suites remain green (unmodified by this branch — `pnpm -r test` passes across every package).

### Additional post-review hardening

A follow-up review of the reasoning/continuation work above found several release-blocking integration gaps before it shipped. Fixed within the same `[0.7.1-alpha]` release, not as a separate version:

- **Finish reasons now fail closed, exhaustively.** `packages/ai-runtime/src/runtime.ts`'s `mapFinishReason()` replaces the old two-case check (`"length"` vs. everything-else-is-success) with an exhaustive switch over WebLLM's own `ChatCompletionFinishReason`, using a compile-time `never` assertion so a future WebLLM finish reason fails to build here rather than silently becoming `"completed"`. `GenerationStopReason` gained two outcomes: `"unsupported_tool_call"` (WebLLM's `"tool_calls"` — FreeAI Open never requests tool use, so this is a response-shape limitation, never model instability, surfaced with its own "this response requires a feature FreeAI Open does not currently support" notice) and `"unknown_terminal"` (the stream ended without WebLLM ever reporting a finish reason at all — failed closed, partial output preserved, never assumed successful).
- **Generation output no longer depends on queued React state.** `AppRuntimeProvider.tsx` now keeps a `GenerationAccumulator` ref per generation (`priorContent`, `generatedDelta`, `mergedContent`, plus the prior status/incompleteReason/continuationCount needed to roll back) updated synchronously from every raw chunk, before any buffered UI render. `persistentGenerationState.ts`'s `setAssistantContent()` (replacing `appendAssistantChunk`) always sets the message to the accumulator's current value rather than appending onto whatever `messages` state happened to already hold, so a delayed/batched React update can never cause the last streamed chunks to go unpersisted.
- **Removed the silent 16,000-character truncation.** `@free-ai-open/conversation-store` and `@free-ai-open/conversation-export` both raise `maxMessageLength` to 64,000 (documented derivation: 12,000 chars/attempt × up to 4 attempts = 48,000 worst case, with margin) and, if content still somehow exceeds that, FORCE the message to `status: "incomplete"` with a new `incompleteReason: "truncated"` and report `truncated: true` back to the caller — never a silent, unsignaled `.slice()`.
- **`cancel_timeout` during Continue is now transactional.** The out-of-band watchdog-error effect reads the same `GenerationAccumulator` and, specifically for `cancel_timeout` mid-continuation, reverts the message to its exact pre-continuation content/status/incompleteReason instead of deleting it (the old, ordinary failed-generation cleanup path). Stop and degenerate-output mid-continuation follow the same revert; a fresh send's `cancel_timeout` behavior (discard the empty placeholder) is unchanged.
- **Context-safe output budgeting.** New `packages/model-router/src/contextBudget.ts`: `calculateGenerationBudget()` enforces `estimatedInputTokens + allowedOutputTokens + safetyMargin <= contextWindow`, using a documented conservative token estimator (no new tokenizer dependency; the estimator itself was hardened further in the second round below, after the initial character-based version proved insufficiently conservative for dense Unicode). Both `sendMessage()` and `continueGeneration()` compute this before calling `runtime.generate()`; `continueGeneration()` refuses to start at all (a new "conversation has grown too long" notice, no attempt consumed) when there is not enough room left for a useful reply. The existing global 2048-token cap remains only a final ceiling, never the context-safety mechanism.
- **`incompleteReason`/`continuationCount` are now persisted end-to-end.** `ConversationMessage` gained both fields (`@free-ai-open/conversation-store` and `@free-ai-open/conversation-export`, fully backward-compatible and round-tripped through export/import). A length-limited reply shows its specific notice after a reload, not only within the generating session; a manual continuation's attempt count survives a refresh, so `MAX_CONTINUATIONS_PER_MESSAGE` (3) cannot be reset by reloading the page.
- **Length-limited generations no longer inflate router confidence.** `ModelPerformanceObservation["outcome"]` (`@free-ai-open/types`) gained `"length_limited"`, `"unsupported_tool_call"`, and `"terminal_unknown"` — all excluded from both the `"completed"` success rate and instability scoring in `model-router`'s `adaptiveObservations.ts` (the same neutral treatment `"cancelled"` already had), replacing the previous behavior of counting a length-limited generation as a plain `"completed"` success.
- **Import now preserves assistant status/reason.** `chat/page.tsx`'s import handler previously dropped `status`/`incompleteReason`/`continuationCount` entirely when creating imported messages; it now passes all three through.
- **Continuation overlap deduplication.** New `apps/web/app/_lib/continuationMerge.ts`: `mergeContinuationOverlap()` finds the longest EXACT suffix/prefix match between the prior content and the newly generated text within a bounded window and removes only that overlap — never a fuzzy/semantic match, never touching a coincidentally repeated line elsewhere in the reply.
- **Reasoning segmentation is now Markdown-aware.** `reasoningSegmentation.ts` computes fenced-code and inline-code-span ranges first and only treats a `<think>`/`</think>` occurrence outside of them as a real reasoning marker — a model showing example markup containing that literal text now renders as code, not a reasoning panel. A real `<think>` tag emitted by the model (never itself inside code) is completely unaffected, including every historical/backward-compatible case already covered.
- **Accessibility.** The reasoning label is wrapped in an `aria-live="polite"` region so a screen reader hears "Thinking…" become "Thinking" without refocusing; `.chat-reasoning__summary` gets a 44px minimum touch target on coarse-pointer/mobile viewports (`globals.css`); the existing global `:focus-visible` rule already covers the summary's focus ring, and no open/close animation exists to gate behind `prefers-reduced-motion`.

### Second-round hardening

A second, independent review of the post-review hardening above found one release-blocking regression and several further correctness gaps before ship. Fixed within the same `[0.7.1-alpha]` release, not as a separate version:

- **Continue no longer discards its own successful result (release blocker).** `continueGeneration()` cleared the generation accumulator (`setActiveGeneration(null)`) BEFORE reading the final merged content from it on the success path, so a successfully-streamed continuation silently fell back to the pre-continuation content in both UI and IndexedDB. The entire Continue attempt (streaming loop, overlap merge, outcome persistence) is now extracted into a plain, directly-testable function — `apps/web/app/_lib/continuationExecution.ts`'s `runContinuationAttempt()` — which returns the final content as a plain value instead of through a ref a caller could clear first, structurally eliminating this bug class. See its mandatory regression test: prior message → Continue → several chunks → natural stop → exact content, verified after an IndexedDB reload, export, and import.
- **`incompleteReason` can now be explicitly cleared.** `updateMessageContent()` previously had no way to distinguish "leave unchanged" from "clear," so a successful continuation could leave the invalid combination `status: "complete"` plus a stale `incompleteReason: "length"`. `UpdateMessageContentInput.incompleteReason` (`@free-ai-open/conversation-store`) now has three-way patch semantics: `undefined` leaves it unchanged, a valid reason sets it, `null` explicitly clears it — the field is genuinely absent from the stored/exported message afterward, not merely `undefined`.
- **Context-token estimation is now a genuine upper bound, not an approximation.** Investigated WebLLM 0.2.84's actual public surface first: `MLCEngine`/`WebWorkerMLCEngine` expose no tokenize/token-count API at all (the `Tokenizer` type from `@mlc-ai/web-tokenizers` is a private field several layers inside `LLMChatPipeline`), confirming a real per-model tokenizer is not reasonably reachable without a large new dependency. `contextBudget.ts`'s `estimateTokenCount()` now returns a string's UTF-8 byte length (`TextEncoder`) instead of `characters / 3`: for every byte-level BPE tokenizer in real use by current open-weight chat models (GPT-2/3/4-style, and LLaMA/Mistral/Qwen-family tokenizers), token count can never exceed byte count, since BPE merging only ever combines bytes into fewer tokens, never splits one byte into more than one — closing a real under-estimation the old character-based formula had for CJK, emoji, and accented text, where a single JS character can be 2-4 UTF-8 bytes. A new `estimateChatInputTokens()` adds an explicit `CHAT_MESSAGE_OVERHEAD_TOKENS` allowance per message segment for chat-template role markers. `GenerationBudgetResult.confidence` is `"conservative"` today, never `"exact"` (no real tokenizer is reachable), rather than the previous unconditional `"low"` — visible groundwork if a real tokenizer ever becomes available. The previously-missing path — no router decision yet available (e.g. before routing resolves on first load) — now goes through the exact same resolver instead of skipping context-checking entirely: new `apps/web/app/_lib/generationBudget.ts`'s `computeBudgetedOutputTokens()` (extracted from `AppRuntimeProvider.tsx` for direct unit testing) assumes the smallest context window any registry model could ever load (`FALLBACK_CONTEXT_WINDOW_TOKENS`, derived from the live registry, not a hardcoded literal) and the global safety cap as the desired output, so `finalOutput = min(desiredOutput, contextSafeOutput, globalSafetyLimit)` now holds on every generation path, not only when a router decision exists.
- **`performanceObservationBuilder` now fails closed for a genuinely absent stop reason.** `classifyGenerationOutcome()`'s final fallback (a `null`/`undefined` stop reason with no runtime error) previously defaulted to `"completed"`; it now reports `"terminal_unknown"`, matching `runtime.ts`'s own fail-closed philosophy — only an explicit `stopReason === "completed"` can ever produce a `"completed"` outcome.
- **Continuation overlap deduplication is materially safer.** `continuationMerge.ts`'s minimum overlap length rose from 3 to 24 characters: a 3-character threshold could delete legitimate short repeated text or code (a shared word like "the", a shared token like "});"), which is strictly worse than leaving a rare, harmless duplicate in place. Below 24 characters, an exact join-point match is now always kept, never trimmed.
- **`<think>` is now only ever recognized at the leading position.** `reasoningSegmentation.ts` previously treated the first `<think>` anywhere outside of code as the reasoning marker; it now requires `<think>` to be the first meaningful content of the assistant's output (ignoring only leading whitespace), matching how Qwen3's chat template actually emits it. A `<think>` appearing later in ordinary prose — e.g. a response explaining raw HTML that happens to contain the literal text `<think>` — now always renders as literal text, whether or not it happens to sit inside a code span; a nested/later `<think>` inside real reasoning is never treated as opening additional hidden reasoning. The existing fenced/inline-code protection for the closing `</think>` search is unchanged, and now has explicit tilde-fence (`~~~`) test coverage alongside the existing backtick-fence coverage.
- **Accessibility: explicit coverage for two properties that were true but unverified.** `ReasoningDisclosure.test.tsx` now asserts the streamed reasoning body is never itself inside the `aria-live` region (only the short, rarely-changing label is — a screen reader must not hear every streamed token), and that the auto-collapse effect contains no focus-manipulating call at all.
- **Local-storage truncation is now clearly surfaced, in both languages, wherever it can happen.** Every caller of `addMessage()`/`updateMessageContent()` now reacts to `truncated: true` instead of some silently ignoring it: a user's own pasted message, a fresh assistant reply, a Continue result, the watchdog's preserved-partial-output path, and conversation import each show a notice distinct from the ordinary "output length limit reached" one — "This response exceeded the maximum size kept locally and was truncated." / "Cette réponse a dépassé la taille maximale conservée localement et a été tronquée." (plus a message-specific equivalent for the user's own truncated input, and an import-summary equivalent). A message already truncated to the storage ceiling is no longer offered the Continue action, since any continuation would just be re-truncated back to the same ceiling.
- **Privacy re-verified for every file this round added or touched**, including a new explicit guard that the continuation/watchdog/context-budget modules introduce no fetch/XHR/WebSocket/`sendBeacon` call of any kind.

See `docs/architecture.md`'s updated "Reasoning output and finish reason", "Reasoning disclosure and Continue", and "Context-safe output budgeting" sections, `docs/DEVLOG.md`'s Sprint 6.21 "post-review hardening" and "second-round hardening" subsections, and this entry's own Fixed/Changed/Security/Tests bullets above (updated in place where they described earlier, superseded behavior) for full detail.
