...
**Status: Phases 0 through 3 are implemented.** The pure adaptive-router core exists, but runtime model-selection integration and router UI do not; the active v0.6 router and fixed compatibility model still power the application today.
...
The fixed runtime default changed from the tiny Phase 0 test model to the verified `SmolLM2-360M-Instruct-q4f32_1-MLC` compatibility record. This is still one deterministic default for every task and mode; Phase 4 will be responsible for translating a future `RouterDecision` into an informed model load with explicit download UX.
