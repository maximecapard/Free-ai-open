...
**Status: Phases 0 through 4 are implemented.** `AppRuntimeProvider` now computes a `RouterDecision` before the first model load and applies it — including safe model switching and real observation recording. Router UI (manual override, richer in-chat explanations) remains Phase 5; the legacy v0.6 router is now used only by `/debug`'s independent side-by-side preview.
...
- **Manual override.** `RouterInput.manualModelId` stays wired through every function used above but is always `undefined` from the app layer this phase — Phase 5 owns the override UI, per the mission's "do not build final UI beyond minimal integration states."
