...
## v0.7.1-alpha (reasoning presentation and finish-reason handling)

- [ ] Run `packages/ai-runtime/src/runtime.test.ts`, `apps/web/app/_lib/reasoningSegmentation.test.ts`, `continuationEligibility.test.ts`, `continuationPrompt.test.ts`, `apps/web/app/_components/ReasoningDisclosure.test.tsx`, and `ChatTranscript.test.tsx`; confirm all pass, including the BeautifulSoup regression fixture and the source-scan guard against logging reasoning content.
- [ ] Ask a reasoning-enabled local model (a Qwen3-family record in the registry) a question; confirm a collapsible "Thinking…" panel appears while the reply streams, with no raw `<think>`/`</think>` text visible anywhere, and that it auto-collapses to "Thinking" once the final answer starts — then confirm manually reopening it afterward works and stays open (the auto-collapse must not re-fire).
- [ ] Force (or wait for) a generation to exhaust its output budget while still inside `<think>` — a long, open-ended reasoning-heavy prompt on a Qwen3 model in a fresh session/profile is a reasonable way to reproduce this; confirm the panel stays expanded, shows "The generation limit was reached."/"La limite de génération a été atteinte." (not the generic notice), and a "Continue"/"Continuer" button appears.
- [ ] Click Continue and confirm the SAME message bubble extends (no new bubble appears) and the reply eventually reaches a real final answer or hits the continuation bound; confirm clicking Continue more than 3 times on one message stops offering it.
- [ ] Click Stop while a Continue is in progress; confirm the message reverts to its pre-continuation content (not deleted, not left half-updated), the existing "generation stopped, not saved" notice appears, and the runtime recovers normally.
- [ ] Import a real historical conversation export containing an old, unclosed `<think>` block persisted with no `status` field (or use `freeai-open-conversation-*.json` from a real bug report if available) and confirm it renders with the generic interrupted notice and a working Continue button with zero manual data migration.
- [ ] Import an old conversation with a closed `<think>...</think>` block and confirm it renders collapsed with only the final answer visible by default, exactly as before this feature existed.
- [ ] Export a diagnostic report during/after a reasoning generation and confirm it still contains no prompt/response/reasoning content, matching the existing content-exclusion guarantee.
- [ ] French/English: switch languages and confirm the "Thinking…"/"Réflexion en cours…", "Thinking"/"Réflexion", both interrupted notices, and the Continue button are all correctly localized.
- [ ] Confirm a non-reasoning model (e.g. a SmolLM2 or Qwen2.5-Coder registry record) shows no reasoning panel at all and behaves exactly as before this release.

## v0.7.1-alpha (post-review hardening: finish-reason fail-closed, accumulator, transactional Continue, context budgeting)

- [ ] Run `packages/ai-runtime/src/runtime.test.ts`, `packages/model-router/src/contextBudget.test.ts`, `apps/web/app/_lib/continuationMerge.test.ts`, `reasoningSegmentation.test.ts`, `generationPipelinePrivacy.test.ts`, and `packages/conversation-store`/`conversation-export`'s test suites; confirm all pass.
- [ ] Reload the page after clicking Continue at least once (whether it completed, was stopped, or hit a watchdog interruption); confirm the continuation attempt count survived the reload (the button stops appearing after exactly 3 total attempts, counting refreshed sessions) and, for a length-limited reply, that the specific "generation limit reached" notice (not the generic one) still shows after the reload.
- [ ] Click Stop during a Continue and inspect the conversation's stored content directly (e.g. via the browser's IndexedDB inspector) to confirm it reverted to EXACTLY the pre-continuation content byte-for-byte, not just visually in the transcript.
- [ ] Ask the local model something likely to produce a fenced code block containing the literal text `<think>` (e.g. "show me an example line that opens a `<think>` tag in HTML") and confirm it renders as code, never as a reasoning panel.
- [ ] Click Continue on a long-reasoning message and confirm the model's own repeated restatement of the prior text (a common "continue from here" behavior) does not appear duplicated in the transcript.
- [ ] Paste a very long block of text as a fresh message on a small-context model (performance mode "fast"/"balanced") and confirm the app still sends a bounded, sensible request rather than crashing or hanging; if the pasted text is long enough to leave no useful room for a reply, confirm the "conversation has grown too long" notice appears instead of starting an unproductive generation.
- [ ] Export a conversation containing a length-limited or continued message, clear local data, and re-import it; confirm `status`, `incompleteReason`, and the continuation count all round-trip exactly.
- [ ] Tab to the reasoning panel's summary with the keyboard and confirm a visible focus ring, and (on a touch device or with a coarse-pointer emulation) confirm the summary is comfortably tappable.

## v0.7.1-alpha (second-round hardening: Continue blocker fix, byte-based context budgeting, storage-truncation UX)

- [ ] Run `apps/web/app/_lib/continuationExecution.test.ts`, `watchdogRecovery.test.ts`, `packages/model-router/src/contextBudget.test.ts`, `apps/web/app/_lib/generationBudget.test.ts`, `continuationMerge.test.ts`, `reasoningSegmentation.test.ts`, `continuationEligibility.test.ts`, `generationPipelinePrivacy.test.ts`, `apps/web/app/_components/ReasoningDisclosure.test.tsx`, `apps/web/app/_i18n/i18n.test.ts`, and `packages/conversation-store`/`conversation-export`'s test suites; confirm all pass.
- [ ] The main regression: prompt a message, wait for it to hit `status: "incomplete"`, click Continue, let it stream to a natural finish, then inspect the conversation directly in the browser's IndexedDB inspector (not just the visible transcript) to confirm the FULL extended content is what was actually saved — not the pre-continuation content. Reload the page and confirm the same. Export the conversation and re-import it into a fresh profile; confirm the imported message still shows the exact full extended content, `status: "complete"`, and no leftover "generation limit reached" notice.
- [ ] Ask the local model something likely to produce a fenced code block containing the literal text `<think>` in ordinary prose (not at the very start of the reply) — e.g. "explain what a `<think>` HTML tag would look like, with an example" — and confirm it renders as plain/code text, never opening a reasoning panel, even outside of a code span.
- [ ] Click Continue on a message and confirm a short repeated word or short code token at the join point (e.g. the model naturally restates "the" or a line ending in "});") is NOT deleted — some visible, harmless repetition at the seam is expected and preferred over losing content.
- [ ] Paste a very long block of text (well beyond a typical message) as a fresh message and confirm that if it is long enough to hit the local 64k storage ceiling, a clear, translated notice appears ("This response exceeded the maximum size kept locally and was truncated." / "Cette réponse a dépassé la taille maximale conservée localement et a été tronquée." — using the message-specific wording for a user's own input) rather than the message silently appearing shorter than what was typed, and that Continue is not offered on a reply already truncated this way.
- [ ] French/English: switch languages and confirm the new storage-truncation notices and the conversation-import truncation summary line are both correctly localized, and read distinctly from the existing "generation limit reached" wording.
