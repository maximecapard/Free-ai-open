...
- [ ] Home "recommended setup": while profiling is pending, `/` shows a detection state instead of a hardcoded `balanced` recommendation; once profiling resolves, clicking "Use the recommended setup" goes straight to a working `/chat` session without visiting onboarding; "Get started" still leads to the full onboarding flow.
- [ ] Home recommendation destinations: with mocked low, ordinary, and strong device profiles, confirm the home CTA routes respectively to `/chat?task=chat&mode=fast`, `/chat?task=chat&mode=balanced`, and `/chat?task=chat&mode=performance`.
...
- [ ] Onboarding recommended mode: on `/onboarding/mode`, the mode matching this device's recommendation shows a "Recommended for this device" badge; the third mode option reads "Quality" (English) / "Qualité" (French), not "Performance".
