...
- [ ] Home "recommended setup": clicking "Use the recommended setup" from `/` goes straight to a working `/chat` session (model loads, no dead task/mode state) without visiting onboarding; "Get started" still leads to the full onboarding flow.
