...
## v0.8.0-alpha local model benchmarking (Phase 0: contracts and architecture only)

Only the local data contract and persistence layer exist in this phase (`@free-ai-open/types`' `ModelBenchmarkResult`, the new `@free-ai-open/model-benchmark` package). No benchmark runner and no benchmarks page exist yet, so none of the runtime-execution notes below are reachable code today — this section pins the security requirements a future runner/UI phase must satisfy, decided before that code is written.
