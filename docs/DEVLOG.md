...
- Kept a wholly separate, much larger emergency safety cap (10 minutes) for a truly pathological runaway generation, under its own distinct `generation_exceeded_safety_limit` error code — never `generation_stalled`, and the old `generation_timeout` code is retired. It is classified as a neutral `cancelled` observation, excluding the app-enforced cutoff from both router success and failure scoring.
- A genuine stall/safety-limit interruption that already produced visible assistant text now preserves and saves that partial reply with durable `status: "incomplete"` provenance; the marker survives refresh and local export/import. An interruption with no output at all still discards the empty bubble, matching prior behavior.
- Forced timeout recovery now rejects late worker chunks before they enter output processing and retains the exact forced error per generation epoch until the abandoned stream can observe it. An app-level fallback still finalizes streams that never settle.
- Conversation writes use atomic store updates, so a late assistant append cannot recreate a conversation deleted at the same time; failed partial-response persistence is reported through the existing storage notice.
...
### Root cause (reasoning presentation and finish reason)

A second, independently reported issue, from a real user-exported diagnostic and conversation (`freeai-open-diagnostic-*.json`/`freeai-open-conversation-*.json`): a `qwen3-4b-q4f16_1-mlc` generation (performance mode, ~162.7s, ~3.1 tokens/s) persisted ending abruptly inside an unclosed `<think>` reasoning block, with no final answer after it, yet the local diagnostic only ever recorded `inference.completed` and no error.

Investigating without assuming the cause: WebLLM's own `ChatCompletionFinishReason` type (`node_modules/@mlc-ai/web-llm`) is `"stop" | "length" | "tool_calls" | "abort"`, and its docs state `finish_reason: "length"` "indicates the generation exceeded `max_tokens`... or the conversation exceeded the max context length." `packages/ai-runtime/src/runtime.ts` read `choice.finish_reason` only to check for `"abort"` — every other value, `"length"` included, fell through to the same `"completed"` outcome the runtime reports for a genuine natural stop. Back-calculating the real conversation's token count from its own recorded `tokensPerSecond`/`generationDurationMs` landed close to 512 tokens, not the "performance" preset's expected 1024/768 — consistent with `@free-ai-open/model-router`'s `hasStrongPerformanceEvidence()` (existing, unchanged logic) falling back to the "balanced" preset (512 output tokens) for this early-session generation, despite `performanceMode: "performance"` being selected. A second, separate global cap, `GENERATION_SAFETY_LIMITS.maxTokens` in `packages/ai-runtime/src/generation-safety.ts` (768), was also silently re-clipping even the full "performance" preset's budget for 3 of the 5 registry models — a second, previously undocumented ceiling.

### Fixed (reasoning presentation and finish reason)

- Extended `GenerationStopReason` (`packages/ai-runtime/src/types.ts`) with a new, explicit `"length"` value, distinct from `"completed"`, `"cancelled"`, and `"degenerate_output"`. `runtime.ts` now captures the real `finish_reason` from whichever stream chunk carries it and reports `"length"` accordingly; the runtime still returns to `ready` (WebLLM itself did not error).
- Raised `GENERATION_SAFETY_LIMITS.maxTokens` from 768 to 2048 so the router-recommended budget (see below) is not immediately re-clipped by this second, previously-undocumented cap.
- Added `packages/model-router/src/adaptiveOutputBudget.ts`: a flat `+512`-token allowance added on top of the selected preset's `maxOutputTokens`, but only for candidates whose registry `family` reasons by default (`Qwen3` currently — `SmolLM2`/`Qwen2.5-Coder` are unaffected). Deliberately narrow: no registry schema change, no change to `adaptiveRouter.ts`'s preset/model eligibility or scoring, only the output-token budget number for models that actually need reasoning headroom.
- `apps/web/app/_lib/generationPersistence.ts`: `isIncompleteAssistantOutput`/`shouldDiscardPartialAssistantOutput`/`generationNoticeKey` all treat `"length"` the same way as a genuine stall/safety-limit interruption with partial output — preserved, marked `status: "incomplete"`, never discarded and never silently accepted as a normal completed reply. `apps/web/app/_lib/performanceObservationBuilder.ts` classifies a length-limited generation as its own neutral `"length_limited"` outcome (not `stalled`, and -- after "Post-review hardening" below -- no longer `completed` either, once the router-confidence implication of that classification was found), per the explicit requirement not to penalize a model merely for exhausting its output budget.
- Added `apps/web/app/_lib/reasoningSegmentation.ts`: a pure string scan (not a general HTML/XML parser — `<think>` is a single, well-known, never-nested Qwen3 marker) splitting raw assistant content into `{beforeReasoning, reasoning, reasoningOpen, afterReasoning}`. Presentation-layer only; the persisted/exported message content is always the original raw string.
- Added `apps/web/app/_components/ReasoningDisclosure.tsx`: a native `<details>`/`<summary>` panel, expanded by default whenever reasoning is open (streaming or interrupted, so partial reasoning is never hidden by default), showing "Thinking…"/"Réflexion en cours…" while the message is the active generation and the static "Thinking"/"Réflexion" label otherwise. Collapses itself exactly once, automatically, on the `reasoningOpen` true-to-false transition (a `useRef` tracking the previous value), without fighting a later manual toggle — the same pattern already used for the code-block copy button's transient state. Reasoning renders through the same safe Markdown renderer as the final answer. `ChatTranscript.tsx`'s `ChatMessageBubble` now segments assistant content and renders `beforeReasoning`/the disclosure/`afterReasoning` separately, and suppresses the generic incomplete-response label when the reasoning-specific notice already explains the interruption.
- Added an interrupted-reasoning notice (shown only once a `<think>` block is open and the message is no longer the active generation): a generic notice for every case, and a length-specific notice shown only when the reason is actually known. Originally tracked as an in-memory-only flag (never persisted); a post-review pass (see "Post-review hardening" below) made this durable as `ConversationMessage.incompleteReason`, so the specific notice now survives a reload too, not only the generating session.
- Added a manual Continue action (`apps/web/app/_runtime/AppRuntimeProvider.tsx`'s `continueGeneration`): reuses the same `assistantMessageId` with a new `generationId` so `persistentGenerationState.ts`'s existing append-onto-existing-content behavior extends the same message bubble instead of creating a duplicate one. `apps/web/app/_lib/continuationPrompt.ts` builds the prompt sent to the model (the original request plus the partial reply plus an explicit "do not repeat/restart" instruction, in the UI's current language) since `runtime.generate()` only ever receives one prompt string per call and does not read prior turns from conversation history — this composed prompt is sent to the model only and is never itself persisted. Persisted via a new `updateMessageContent()` (`@free-ai-open/conversation-store`, replaces one message's content/status in place; a no-op for an unknown message ID) rather than `addMessage()`, which is strictly append-only and would otherwise create a duplicate. Bounded to `MAX_CONTINUATIONS_PER_MESSAGE = 3` (a manual click is preferred over an invisible automatic retry, per the explicit requirement, but the click is still bounded). Cancelling mid-continuation reverts the message to its pre-continuation content (mirroring `shouldDiscardPartialAssistantOutput`'s existing unconditional-discard rule for a cancellation, adapted so the message itself — unlike a fresh reply — is never removed, since it already held a legitimate prior answer).
- Added `apps/web/app/_lib/continuationEligibility.ts`'s `isMessageContinuable()`: a message is eligible for Continue when it carries an explicit `status: "incomplete"`, OR — for a message persisted before that status field existed, including every real historical conversation exported before this feature shipped — its `<think>` block is still open with no generation currently active. Found directly from manually verifying against the real reported conversation: gating Continue on `status === "incomplete"` alone left every pre-existing conversation, including the one that reported this bug, without a Continue button at all.
- Fixed the out-of-band watchdog-error effect in `AppRuntimeProvider.tsx` (the fallback path for a forced-recovery error whose stream never delivers its own terminal error chunk) to use `updateMessageContent` instead of `addMessage` when the interrupted generation was a continuation (tracked via `continuationInProgressRef`, set at the start of `sendMessage`/`continueGeneration` and never reset mid-flight, avoiding a race between an async watchdog effect and the generation's own cleanup) — otherwise a watchdog interruption landing mid-continuation would have created a duplicate message.
- Inspected WebLLM's own `enable_thinking` option (prepends `<think>\n\n</think>\n\n` to suppress reasoning tokens when `false`). Documented in `docs/architecture.md`'s "Reasoning output and finish reason" section as a real, available lever for a future release; deliberately not wired up here, since this hotfix's scope is presenting and budgeting for reasoning correctly, not deciding whether to request it.

### Tests (reasoning presentation and finish reason)

- Extended `runtime.test.ts` with the natural-stop-vs-length distinction, `"length"` classification, and a regression fixture reproducing the real bug (a generation whose entire output budget is spent inside an unclosed `<think>` block).
- Added `reasoningSegmentation.test.ts` and `continuationEligibility.test.ts` (malformed/edge-case tag handling; explicit-status vs. legacy-unclosed-think eligibility, including the exact real historical shape).
- Added `ReasoningDisclosure.test.tsx` and `ChatTranscript.test.tsx`, rendered statically via `react-dom/server` (same approach as `MessageContent.test.tsx`): label/notice states, safe Markdown rendering of reasoning, a user message's literal `<think>` text never parsed as reasoning, generic-label suppression, a real historical raw-`<think>` fixture rendering with no migration, and a source-scan guard against logging reasoning content.
- Added `continuationPrompt.test.ts` and extended `conversation-store`'s `index.test.ts` for `updateMessageContent`.
- Extended `adaptiveOutputBudget.test.ts` (new)/`adaptiveRouter.test.ts` for the reasoning-family budget allowance, and `generationPersistence.test.ts`/`performanceObservationBuilder.test.ts` for `"length"`'s discard/notice/classification rules.
- Manually verified end-to-end against the real reported conversation and diagnostic, imported through the app's own Import action (not a synthetic fixture): historical interrupted message renders correctly with a working Continue button with zero code changes for old data; Continue against the real local runtime extends the same bubble; cancelling mid-continuation reverts cleanly with no duplicate message and zero console errors.

### Remaining limits (reasoning presentation and finish reason)

- The `+512`-token reasoning allowance and the 45s/2048-token safety adjustments remain judgment calls, not measured against a broad model/device matrix.
- `enable_thinking` is documented but not wired up; there is currently no way to request a non-reasoning response from a model that reasons by default.
- Fenced code blocks inside a reasoning panel render with the same renderer as the final answer, but a very long reasoning block has no virtualization/collapsing-within-the-panel — the panel itself is the only collapse point.

### Post-review hardening (finish-reason fail-closed, accumulator architecture, transactional Continue, context budgeting)

A follow-up review of the reasoning/finish-reason work above, before it shipped, found several release-blocking integration gaps. Fixed within the same Sprint 6.21 / v0.7.1-alpha scope:

- **Exhaustive finish-reason mapping.** runtime.ts's mapFinishReason() replaced the old "length vs. everything else is success" check with an exhaustive switch over WebLLM's ChatCompletionFinishReason, guarded by a compile-time never assertion so a new WebLLM finish reason fails to build here instead of silently mapping to "completed". Added "unsupported_tool_call" (WebLLM's "tool_calls"; FreeAI Open never requests tool use) and "unknown_terminal" (stream ended with no finish_reason at all) as their own GenerationStopReason values, both failing closed rather than being assumed successful.
- **Generation accumulator, independent of React state.** AppRuntimeProvider.tsx gained a GenerationAccumulator ref (priorContent, generatedDelta, mergedContent, plus everything needed to roll a continuation back) updated synchronously from every raw chunk before any buffered render. persistentGenerationState.ts's setAssistantContent() replaces appendAssistantChunk(): it always sets the message to the accumulator's current value, never appends onto whatever messages state happens to already hold, so a delayed/batched React update can no longer risk losing the last streamed chunks at persistence time.
- **Removed the silent 16,000-character truncation.** conversation-store and conversation-export raise maxMessageLength to 64,000 (documented derivation: 12,000 chars/attempt x up to 4 attempts) and, if content still exceeds that, force status: "incomplete" + incompleteReason: "truncated" and report truncated: true to the caller instead of silently slicing.
- **Transactional Continue.** The watchdog-error effect now reads the same accumulator and, for cancel_timeout specifically during a continuation, reverts the message to its exact pre-continuation content/status/incompleteReason instead of running the ordinary discard-the-message cleanup path. Stop and degenerate-output mid-continuation revert the same way (already the case before this round, now sharing the accumulator as its data source instead of re-reading messagesRef).
- **Context-safe output budgeting.** New packages/model-router/src/contextBudget.ts's calculateGenerationBudget() enforces estimatedInputTokens + allowedOutputTokens + safetyMargin <= contextWindow using a documented conservative character-based estimator (no new tokenizer dependency, per explicit instruction). Wired into both sendMessage() and continueGeneration(); continueGeneration() refuses to start (no attempt consumed) when there is not enough room left for a useful reply, showing a new "conversation has grown too long" notice. The existing global 2048-token cap is still only a final ceiling.
- **Durable incompleteReason/continuationCount.** Both are now real ConversationMessage fields, round-tripped through export/import, backward-compatible with every message that predates them. A length-limited reply's specific notice and a continuation's attempt count both now survive a page reload.
- **Router scoring no longer rewards length truncation.** ModelPerformanceObservation["outcome"] gained "length_limited", "unsupported_tool_call", "terminal_unknown", all excluded from both the completed-success rate and instability scoring in adaptiveObservations.ts — replacing the previous (incorrect) classification of a length-limited generation as a plain "completed" success.
- **Import fix.** chat/page.tsx's import handler was dropping status/incompleteReason/continuationCount when creating imported messages; it now passes all three through.
- **Continuation overlap dedup.** New continuationMerge.ts's mergeContinuationOverlap() removes only an exact suffix/prefix match at the join point within a bounded window — never fuzzy, never touching a coincidentally repeated line elsewhere.
- **Markdown-aware reasoning segmentation.** reasoningSegmentation.ts now computes fenced/inline code ranges first and only treats a <think>/</think> outside of them as real reasoning markup, so example markup shown inside a code block never opens a reasoning panel; every real, historical, non-code case is unaffected.
- **Accessibility.** aria-live="polite" on the reasoning label; a 44px coarse-pointer/mobile touch target on the summary; focus-visible and reduced-motion were already covered (a global rule and "no animation exists", respectively).

#### Tests (post-review hardening)

- runtime.test.ts: tool_calls, a stream ending with no finish_reason at all, and a synthetic out-of-union value (an intentional unsafe cast in the test only) all resolve to their correct distinct outcome, never "completed".
- contextBudget.test.ts (new): 1024/2048/4096-token contexts, a short vs. long prompt, a reasoning model's larger desired budget, a Continue-sized long prior reply, zero/insufficient remaining budget, and the global cap still applying as a ceiling.
- continuationMerge.test.ts (new): no overlap, one-line and multi-line overlap, the mission's own BeautifulSoup worked example, repeated code elsewhere preserved, empty continuation, Unicode/emoji, a Markdown-fence-spanning overlap, and incremental chunk-by-chunk accumulation converging to the same result as a one-shot merge (proving correctness independent of render/flush timing).
- reasoningSegmentation.test.ts: extended with literal <think> inside a fenced/inline code span, a real tag before/around a code block, an unclosed fence, and a malformed tag inside code.
- conversation-store/conversation-export: a 48,000-character round trip (1 initial reply + 3 continuations) surviving exactly; a message that genuinely exceeds the new ceiling is truncated with truncated: true and forced incomplete/"truncated", on both addMessage and updateMessageContent; incompleteReason/continuationCount round-trip through export/import exactly, and are absent (not defaulted) for a legacy message with neither.
- ReasoningDisclosure.test.tsx: aria-live/aria-atomic on the streaming label; didReasoningJustClose() (the auto-collapse decision, extracted as a pure function) covering all four open/closed transitions, plus a source check that the effect's dependency array is exactly [reasoningOpen] so a manual reopen can never be forced shut again.
- generationPipelinePrivacy.test.ts (new): source-scans the whole generation/continuation pipeline for console.* calls and for any logging call referencing accumulator/continuation content fields.
- Manually verified end-to-end against the real reported conversation, using the real local WebLLM runtime (not a mock): Continue extends the same bubble with correct overlap dedup; Stop mid-continuation reverts to the exact original content (byte-for-byte, confirmed via direct IndexedDB inspection) with continuationCount still durably incremented; reloading the page preserves both the reverted content and the attempt count, and still offers Continue for the remaining attempts.

#### Remaining limits (post-review hardening)

- The context-budget estimator was a character-based heuristic at this point in the review process; a second, independent review found it insufficiently conservative for dense Unicode/CJK/emoji and it was hardened to a byte-length-based genuine upper bound — see "Second-round hardening" below. It is still not a real per-model tokenizer count: confidence: "conservative" is reported explicitly rather than implying a precision the estimate does not have.
- cancel_timeout-during-Continue and "runtime recovery fails"-during-Continue shared the exact same revert code path verified for Stop-during-Continue (confirmed by direct code inspection) but had no independently-triggered test of their own at this point — closed in "Second-round hardening" below (watchdogRecovery.test.ts), without ever needing a real 15-second worker hang: the persistence/rollback decision was extracted into a plain function that takes the already-classified error code as a direct input.
- The new MessageIncompleteReason/continuationCount fields are additive-only; no migration was needed or written, since every reader already treats them as optional.

### Second-round hardening (Continue blocker, byte-based context budgeting, overlap/reasoning correctness, storage-truncation UX)

A second, independent review of the post-review hardening above, before it shipped, found one release-blocking regression and several further correctness gaps. Fixed within the same Sprint 6.21 / v0.7.1-alpha scope:

- **Continue was discarding its own successful result (release blocker).** `continueGeneration()`'s success path called `setActiveGeneration(null)` — which clears the generation accumulator ref as a side effect — BEFORE reading `mergedContent` from that same ref for the final persisted content, so a successfully-streamed continuation always fell back to `priorContent`, silently discarding everything it had just generated, in both UI and IndexedDB. Fixed by extracting the entire attempt into `apps/web/app/_lib/continuationExecution.ts`'s `runContinuationAttempt()`, a plain `async` function that computes and returns `finalContent` as part of its resolved value — there is no shared mutable ref left for a caller to clear before reading, so this exact ordering mistake is now structurally impossible, not just patched. The watchdog's out-of-band rollback/preserve decision was extracted the same way, into `apps/web/app/_lib/watchdogRecovery.ts`'s `runWatchdogRecovery()`.
- **`incompleteReason` can now be explicitly cleared.** `updateMessageContent()` previously had no way to express "remove this field" — only "leave it" or "set it to a new value" — so a successful continuation could leave `status: "complete"` next to a stale `incompleteReason: "length"` from before the continuation ran. `UpdateMessageContentInput.incompleteReason` (`@free-ai-open/conversation-store`) is now three-way: `undefined` = unchanged, a valid reason = set, `null` = clear. The message is rebuilt field-by-field rather than via `{...message, ...patch}` spreading specifically so a cleared field is genuinely absent from the resulting object (`Object.prototype.hasOwnProperty` false), not merely `undefined`.
- **Context-token estimation is now a genuine upper bound.** Investigated WebLLM 0.2.84's actual public API surface first (per explicit review instruction): `MLCEngine`/`WebWorkerMLCEngine` expose no tokenize/token-count method at all; the private `Tokenizer` inside `LLMChatPipeline` is unreachable from `ai-runtime`'s public surface without a large new dependency. Replaced `estimateTokenCount()`'s `characters / 3` with a string's raw UTF-8 byte length (`TextEncoder`) — provably an upper bound on token count for any byte-level-BPE-family tokenizer (GPT-2/3/4-style, LLaMA/Mistral/Qwen), since BPE merges only ever combine bytes into fewer tokens, never split one byte into more than one. The old formula under-counted CJK/emoji/accented text badly (a CJK character is 3 UTF-8 bytes but counted as 1 character under the old rule). Added `estimateChatInputTokens()` + `CHAT_MESSAGE_OVERHEAD_TOKENS` for per-message chat-template overhead, and `confidence: "conservative"` (renamed from an unconditional `"low"`; `"exact"` is reserved terminology for a real tokenizer that does not exist in this codebase today). Closed the one remaining unguarded generation path: no router decision yet available previously skipped context-checking entirely (`maxOutputTokens: undefined`, falling back to the raw 2048-token global cap with zero context awareness); `apps/web/app/_lib/generationBudget.ts`'s `computeBudgetedOutputTokens()` (extracted from `AppRuntimeProvider.tsx`) now resolves this case too, against `FALLBACK_CONTEXT_WINDOW_TOKENS` (the live registry's smallest context preset) and the global cap as the desired output.
- **`performanceObservationBuilder` fail-closed gap.** `classifyGenerationOutcome()`'s final fallback (no stop reason, no error) defaulted to `"completed"` — contradicting the fail-closed philosophy the rest of the runtime already follows. Now returns `"terminal_unknown"`; only an explicit `stopReason === "completed"` can ever produce `"completed"`.
- **Overlap dedup minimum raised 3 -> 24 characters.** 3 characters is far too short a bar: a shared word ("the") or a shared code token ("});") is not reliably a duplicated-continuation artifact, and deleting it destroys real content — strictly worse than an occasional harmless duplicate. Below 24 characters an exact join-point match is now always kept.
- **Reasoning `<think>` recognized only at the leading position.** Previously the first `<think>` anywhere outside of code was treated as the reasoning marker; a response explaining raw HTML containing the literal text `<think>` later in ordinary prose (not necessarily inside a code span) could be mistaken for hidden reasoning. Now requires `<think>` to be the first meaningful content of the assistant's output (leading whitespace ignored), matching how Qwen3's own template actually emits it — which also structurally subsumes the old "outside of code" check for the open tag. A later/nested `<think>` is always literal text, never additional hidden reasoning. Closing-tag code-awareness (fenced + inline, backtick and tilde) is unchanged; added the explicit tilde-fence test that was previously claimed-but-unverified.
- **Local-storage truncation UX.** Audited every `addMessage()`/`updateMessageContent()` caller for `truncated: true`; three call sites were reusing the "output length limit reached" notice for storage truncation (a materially different, misleading situation — Continue is exactly the wrong suggestion for an already-truncated message), and the user's own outgoing message and conversation import were not checking `truncated` at all. Added `storageNotice.generationTruncatedForStorage`/`storageNotice.messageTruncatedForStorage`/`backup.messageTruncated` (EN/FR) and excluded `incompleteReason: "truncated"` from `isMessageContinuable()`.
- **Accessibility: explicit coverage for two properties that were true but unverified.** Added a test proving the streamed reasoning body never sits inside the `aria-live` region (only the short, rarely-changing label does), and a structural test that the auto-collapse effect contains no focus-manipulating call.
- **Privacy re-verified** for every file this round added (`continuationExecution.ts`, `watchdogRecovery.ts`, `generationBudget.ts`), plus a new explicit guard that none of them ever perform a `fetch`/XHR/WebSocket/`sendBeacon` call.

#### Tests (second-round hardening)

- `continuationExecution.test.ts` (new): the mandatory regression (prior message -> Continue -> several chunks -> natural stop -> exact content, verified after an IndexedDB reload, export, AND import), a length-limited continuation staying incomplete, Stop/degenerate-output reverting exactly, abandonment when superseded, and onProgress receiving deduplicated (not raw) content.
- `watchdogRecovery.test.ts` (new): cancel_timeout during Continue restoring exact prior content/status/reason and requesting runtime recovery; the same with a genuinely-absent prior reason; a failed persistence write during recovery still returning the correct reverted values (never "remove"); a genuine stall preserving accumulated progress; a fresh send with/without output discarding vs. persisting correctly. All deterministic, plain function calls — no timers needed, since the error code is already-classified input.
- `contextBudget.test.ts`: rewritten estimator tests (byte-length invariants against plain ASCII, accented French, CJK, emoji, punctuation, source code, Markdown fences) proving the estimate never falls below a naive one-token-per-character count; `estimateChatInputTokens` overhead tests; an end-to-end adversarial CJK+emoji+code budget test.
- `generationBudget.test.ts` (new): Qwen3 Fast/Balanced/Performance-shaped contexts, a long-Continue-vs-short-prompt comparison, and the two regression cases for the previously-unguarded missing-router-decision path (real context budgeting now applies; a too-long prompt now correctly reports insufficient context, which was impossible before this fix).
- `continuationMerge.test.ts`: added the "the"/"});"-style short-overlap-kept cases, exact-boundary (23 vs. 24 character) tests, a substantial accented-Unicode overlap, and an overlap spanning an emoji surrogate pair.
- `reasoningSegmentation.test.ts`: leading-whitespace-tolerant detection, text-before-`<think>`-disqualifies-it, the raw-HTML-example-in-prose case, a nested/later `<think>` never reopening reasoning, malformed leading tags, and explicit tilde-fence coverage for both the open-tag and closing-tag-inside-code cases.
- `ReasoningDisclosure.test.tsx`: added the aria-live-scoping test (streamed content excluded) and the focus-safety structural test.
- `continuationEligibility.test.ts`: a truncated, otherwise-"incomplete" message is correctly not continuable.
- `conversation-store`/`conversation-export`: added reload verification to the existing truncation tests, and a new already-truncated-message export/import round-trip test.
- `generationPipelinePrivacy.test.ts`: extended to the three new files, plus a new no-network-call guard.
- `i18n.test.ts`: exact wording assertions for the new truncation notices, including the mandated French copy.
- Full monorepo `pnpm -r typecheck`, `pnpm -r test`, `pnpm lint`, and `pnpm build` all green; `git diff --check` clean (no real whitespace errors).

#### Remaining limits (second-round hardening)

- `estimateTokenCount()` is a proven upper bound for byte-level-BPE-family tokenizers, not an exact per-model count — no public WebLLM tokenizer API exists to compare against or fall back from; "exact" confidence remains unreachable code until one does.
- A full React mount/rerender/click integration test for `ReasoningDisclosure`'s auto-collapse (vs. the pure `didReasoningJustClose()` unit coverage plus a static-property source check) and a live browser accessibility smoke pass both remain out of scope — this repo still has no DOM/React-testing-library dependency, and a browser smoke pass for this specific micro-interaction would require loading a real local model, which is not practical in this environment.
- `chat/page.tsx`'s import-flow truncation surfacing (`backup.messageTruncated`) is verified at the `conversation-store`/`conversation-export` layer it calls into, not as a mounted-page integration test, for the same no-DOM reason as every other page-level component in this app.

## Sprint 6.22 - v0.8.0-alpha Phase 0: local model benchmark contracts

### Built

- Defined the "Local Benchmarks & Performance Intelligence" v0.8.0-alpha contracts in `@free-ai-open/types` (`benchmark-signals.ts`, a new file separate from `router-signals.ts`): `ModelBenchmarkPreset` ("quick"/"standard"/"extended"), `ModelBenchmarkContextPreset` (mirrors `@free-ai-open/model-registry`'s `contextPresetIds` by value, duplicated rather than imported so this leaf package stays dependency-free), `ModelBenchmarkOutcome` (deliberately the exact same type as `ModelPerformanceObservation["outcome"]`, not a parallel union), and `ModelBenchmarkResult` with its sub-shapes (`ModelBenchmarkModelReference`, `ModelBenchmarkRunConfig`, `ModelBenchmarkLoadMeasurement`, `ModelBenchmarkFirstTokenMeasurement`, `ModelBenchmarkEnvironment`). `ModelBenchmarkGenerationMeasurement` is a discriminated union (`ModelBenchmarkGenerationMeasurementExact` / `...Unavailable`) rather than one flat interface with optional fields, so an "exact" measurement missing its count, its duration, or its rate is unrepresentable at the type level, not merely invalid data to catch later.
- This is explicitly NOT the existing generic device/WebGPU compute microbenchmark (`LocalBenchmarkResult`, Sprint 6.16/v0.7.0-alpha Phase 2, unchanged by this work) — `ModelBenchmarkResult` measures one actual local MODEL's real load/generation behavior, the same category `ModelPerformanceObservation` already answers from incidental real usage, but from a deliberate, bounded, user-initiated run.
- Added the new `@free-ai-open/model-benchmark` package (depends only on `@free-ai-open/types`, the same "shared contract in a zero-dependency leaf package" pattern Sprint 6.13 established): `store.ts`/`indexed-db-store.ts`/`memory-store.ts` (an IndexedDB-backed `ModelBenchmarkStore` with a `modelId` index, plus an in-memory fallback, mirroring `@free-ai-open/conversation-store`'s architecture exactly — neither backend is exported from the package's public `index.ts`, so `ModelBenchmarkStoreClient` is the only externally reachable way to persist or read a result), `validation.ts`'s `sanitizeModelBenchmarkResult()` (schema/shape validation plus cross-field invariants on every write and read, re-run independently inside each storage backend as defense in depth), `expiry.ts`'s `calculateModelBenchmarkExpiry()` (the one canonical `createdAt + TTL` formula), `pruning.ts`'s `computeModelBenchmarkIdsToPrune()` (one pure eviction-selection function shared by both backends), `compatibility.ts` (`isModelBenchmarkResultExpired`/`isModelBenchmarkResultCompatible`/`isModelBenchmarkResultUsable`/`filterUsableModelBenchmarkResults`), `evidence.ts`'s `summarizeModelBenchmarkEvidence()` (a pure, count-gated evidence-strength aggregation contract for a future router phase), `stability.ts`'s `classifyModelBenchmarkStability()` (positive/neutral/negative, mirroring `model-router`'s own `NEUTRAL_OUTCOMES` set exactly), and `client.ts`'s `ModelBenchmarkStoreClient` (migration-on-read, `clearAll()`/`clearForModel()`, and a bounded history — 100 results total, 20 per model, oldest dropped first — enforced atomically inside each backend's own `putAndPrune()`, never as a separate step after the write).
- Defined exact metric semantics rather than leaving them implicit: load time is model-load-request-to-ready as a single number (download time is not split out separately since it cannot be measured consistently across browsers/CDN conditions), with `wasModelCachedBeforeRun`/`modelLoadedDuringRun` distinguishing a fresh download from a cached load from no load at all (and `sanitizeModelBenchmarkResult()` rejects a `loadTimeMs` on a run that was not actually loaded, since that would be claiming a measurement that could not have occurred); time to first token is inference-start to the earliest raw runtime token, independent of React/buffered rendering; generation throughput is `tokenCountConfidence: "exact" | "unavailable"` with no approximate middle tier — an "exact" count must be a non-negative integer, the duration strictly positive, and the claimed rate numerically consistent with count/duration within a small explicit tolerance, so a character-length-derived or otherwise fabricated tok/s is rejected outright rather than merely discouraged by convention.
- Investigated the existing `@free-ai-open/local-benchmark` (generic device benchmark) and `apps/web/app/_lib/modelObservationStore.ts` (incidental usage observations) before deciding NOT to overload either: the device benchmark is a conceptually different measurement (raw device throughput, no model involved), and the observation store's own per-field-drop leniency (an invalid optional metric is silently dropped, the rest of the record kept) is the wrong policy for a benchmark result specifically, which is deliberate/infrequent/higher-stakes evidence — `sanitizeModelBenchmarkResult()` is deliberately STRICTER: any present-but-invalid optional metric (a negative or non-finite timing) rejects the whole record.
- Updated `docs/roadmap.md`, `docs/architecture.md`, `docs/privacy.md`, `docs/security.md`, and `CHANGELOG.md` with the new contract, metric semantics, persistence architecture, defined-but-not-yet-wired router relationship, and privacy guarantees. Explicitly marked contracts/architecture only everywhere — no benchmark runner, no benchmarks page.
- A subsequent review of this same Phase 0 pass (before any of it was committed) found several correctness/privacy gaps between what the docs above claimed and what the code actually enforced. The bullets below and the "Privacy and architecture notes"/"Tests" sections describe the corrected, review-hardened version of this sprint's contract — not a separate follow-up sprint, since nothing from the initial pass had shipped yet.

### Privacy and architecture notes

- `ModelBenchmarkResult` has no field capable of holding prompt text or generated response text — only bounded technical measurements and the same coarse references (`capabilityProfileKey`, `browserFamily`) already used elsewhere in this codebase, never a raw GPU adapter string, unique fingerprint, or full capability profile. `sanitizeModelBenchmarkResult()` rebuilds a brand-new object from an exact allowlist rather than trusting the input object's own shape, and every storage backend re-runs this same check independently before its own write.
- `capabilityProfileKey` is validated against the exact same low-entropy, four-segment grammar `buildCapabilityProfileKey()` produces (form factor / capability class / WebGPU availability / adapter kind, each from its own small allowlist) rather than merely a bounded-length string — an arbitrary high-entropy value, a UUID, or an exact GPU adapter description can never be persisted.
- `@free-ai-open/model-benchmark` introduces no new inter-package dependency edge touching `model-router`, `ai-runtime`, or `model-registry`; it depends only on `@free-ai-open/types`. `apps/web/app/_lib/packageDependencyBoundaries.test.ts` now asserts this directly.
- Compatibility/invalidation is exact-match-only by design: `isModelBenchmarkResultCompatible()` requires this package's own current `benchmarkVersion`, the CURRENT registry version, WebLLM version, capability-profile key, AND browser family to all match a stored result's recorded ones — any single mismatch (or expiry) means the result is ignored outright, never down-weighted, matching the router-relationship rule this Phase 0 defines (but does not wire) for a future phase. `expiresAt` must equal `createdAt + TTL` exactly (`calculateModelBenchmarkExpiry()`), so a forged far-future expiry can never extend a result's usable lifetime.
- Stability classification (`classifyModelBenchmarkStability()`) is a pure function computed fresh from `outcome`, deliberately never persisted on the result itself, so a future refinement of the rules applies retroactively to every already-stored result without a migration. `summarizeModelBenchmarkEvidence()` reuses this same classification to tally stable/neutral/negative runs, so a neutral outcome (e.g. a user cancellation) can never inflate a failure count in the aggregate either.
- Stage/outcome/measurement legality is enforced by an explicit lookup table plus cross-field checks (`not_started` only ever pairs with `cancelled`; a load failure can never coexist with generation metrics; a stage implying at least one token requires `firstTokenTimeMs`; `complete` always has a `generationDurationMs`) — a deliberate validation-layer alternative to a full discriminated-union rewrite of `ModelBenchmarkResult` itself, narrower in scope for the same guarantee.
- The IndexedDB backend inserts a result and enforces the history cap inside one atomic readwrite transaction, resolving only on `transaction.oncomplete` and rejecting on `onabort`/`onerror` — never reporting success from a request's own `onsuccess` alone, and never a state where the insert succeeded but pruning did not.

### Tests

- `benchmark-signals.test.ts`: contract-shape tests for `ModelBenchmarkResult` (every outcome value, an already-loaded-model load measurement with no timing, an unavailable-confidence generation measurement with no derived rate, a load-failure result with no generation/first-token measurements) plus the existing "no prompt/response/conversation-shaped fields" contract-test convention, plus compile-time `@ts-expect-error` proof tests that an "exact" measurement missing its count/duration/rate, or an "unavailable" one carrying a count, cannot be constructed at all.
- `stability.test.ts`: every outcome correctly classified positive/neutral/negative, explicitly including that a user cancellation is neutral (never penalizes the model), and a fail-closed default for an outcome outside every known group.
- `validation.test.ts`: a fully valid result accepted unchanged and deterministically (repeated sanitization is byte-for-byte identical); rejection of an unsupported past/future schema version or `benchmarkVersion`, invalid/future `createdAt`, an `expiresAt` that does not exactly match the canonical TTL formula (including a forged far-future value), an impossible/unknown outcome/stage/preset, a negative/non-finite timing on any optional metric, a top-level/nested `modelId` mismatch, an invalid `tokenCountConfidence`, and a non-semver `webllmVersion`; the full exact-throughput invariant matrix (missing/fractional/negative count, missing/zero duration, NaN/Infinite or inconsistent rate, the zero-tokens-implies-zero-rate rule, large-but-consistent values accepted); the capability-key grammar (every legitimate combination accepted; a UUID, an exact GPU description, a long hardware signature, a wrong segment count, an out-of-allowlist segment, and a casing mismatch all rejected); the stage/outcome/measurement legality table (`not_started`+`completed`, a load failure with generation metrics, a reused model with a `loadTimeMs`, and several more all rejected; a cancellation before or during inference explicitly allowed with well-defined partial metrics); confirmed `generatedTokenCount`/`generationTokensPerSecond` are dropped (not rejected, and not even present as properties on the resulting object) when confidence is "unavailable" even if present in the raw input; confirmed injected `prompt`/`response`/`content`/`reasoning`/`messages`/`systemPrompt`/`generatedText`/`rawOutput`/other unknown top-level and nested fields never survive sanitization.
- `compatibility.test.ts`: expiry at/after/before the exact instant; compatibility requiring all five signals (benchmark version, registry version, WebLLM version, capability-profile key, browser family) to match, with a single-signal mismatch enough to invalidate; `filterUsableModelBenchmarkResults()` filtering a mixed list down to only usable results.
- `evidence.test.ts`: evidence level is count-gated (a single sample is always "weak", never stronger; exact threshold boundaries for "moderate"/"strong"); stable/neutral/negative tallies never conflate a neutral outcome with a failure; most-recent timestamp picks the true maximum regardless of input order; median load/first-token/throughput aggregation (odd/even sample counts, throughput restricted to exact-confidence samples only, `null` when no sample carries a measurement).
- `client.test.ts`: record/list round-trip; a malformed result rejected without persisting; per-model filtering; `clearAll()`/`clearForModel()` leaving other models' history intact; per-model history-cap enforcement (oldest dropped first, atomic with the insert); migration-on-read silently dropping an already-corrupted stored record (simulated by writing directly to the raw IndexedDB object store, since every sanctioned write path now rejects it); one real IndexedDB-backed test via `fake-indexeddb`, mirroring `@free-ai-open/conversation-store`'s own `withFakeIndexedDb` pattern.
- `apps/web/app/_lib/packageDependencyBoundaries.test.ts`: extended with a `@free-ai-open/model-benchmark` boundary describe block (depends on nothing but `types`, no edge to `model-router`/`ai-runtime`/`model-registry`, no cycle).

### Known limitations after Sprint 6.22

- No benchmark runner exists yet — nothing in the app actually produces a `ModelBenchmarkResult`. `@free-ai-open/model-benchmark`'s persistence/validation/compatibility/evidence layer is fully implemented and tested, but unwired.
- No `/benchmarks` page exists yet.
- The adaptive router does not read `ModelBenchmarkResult` yet; "Router relationship" in `docs/architecture.md` defines the rules a future wiring phase must follow, but `routeAdaptiveModel()` itself is unchanged.
- `ModelBenchmarkContextPreset`'s values are hand-duplicated from `@free-ai-open/model-registry`'s `contextPresetIds` rather than imported (this package must stay dependency-free) — a future change to the registry's own preset ids would need a matching manual update here, guarded only by `benchmark-signals.test.ts`'s literal-value assertion, not a compile-time link.
- `MODEL_BENCHMARK_EVIDENCE_THRESHOLDS` (weak/moderate/strong sample-count cutoffs) are a conservative, reviewable starting point, not derived from real usage data — a future router-integration phase may need to tune them once "quick" has real evidence of its own.

### Planned work (not implemented yet)

- Phase 1: runtime-backed performance metrics — see Sprint 6.23 below (now complete).
- Phase 2: benchmark runner — drive `@free-ai-open/ai-runtime` through one bounded "quick"-preset run (now producing real `GenerationRuntimeMetrics`, per Sprint 6.23) and translate it into a persisted `ModelBenchmarkResult`.
- Phase 3: `/benchmarks` page — consumer-friendly presentation ("Model load time", "Time to first response", "Generation speed", "Stability"), technical fields on demand only.
- Phase 4: router integration — benchmark evidence as additional router input, never a hard-eligibility override, never dominating long-term observation-based stability scoring.
- Phase 5: `standard`/`extended` presets, if still wanted once "quick" has real usage evidence of its own value.

## Sprint 6.23 - v0.8.0-alpha Phase 1: runtime-backed performance metrics

### Built

- **Verified the installed WebLLM version and its real API before writing any code.** `@mlc-ai/web-llm` 0.2.84 (per `packages/ai-runtime/package.json`/`pnpm-lock.yaml`) — read `lib/openai_api_protocols/chat_completion.d.ts`/`types.d.ts` directly rather than assuming from memory or generic WebLLM docs (which can describe a different version). Findings, recorded in `runtime.ts`'s own top-of-file comment: `ChatCompletionRequestBase.stream_options: {include_usage: true}` is fully supported (`ChatCompletionRequestUnsupportedFields` is an empty array in this version); setting it causes a dedicated trailer chunk (`choices: []`, a populated `usage: CompletionUsage`) after the normal stream end, carrying real tokenizer-backed `completion_tokens`/`prompt_tokens`/`total_tokens` computed by WebLLM's own pipeline; `CompletionUsage.extra` additionally exposes WebLLM's own self-measured `prefill_tokens_per_s`/`decode_tokens_per_s`/`e2e_latency_s` (deliberately not used here — see below).
- Extended `@free-ai-open/ai-runtime`'s `types.ts` with `GenerationTokenUsageExact`/`GenerationTokenUsageUnavailable`/`GenerationTokenUsage` (a discriminated union mirroring Phase 0's own exact-or-unavailable philosophy, but package-local — not imported from `@free-ai-open/types`), `GenerationRuntimeMetrics` (inference-start/first-token/completion timestamps, TTFT, generation duration, and the usage above), and `ModelLoadRuntimeMetrics` (`loadTimeMs`). `GenerateChunk`'s `"done"` variant now carries `metrics: GenerationRuntimeMetrics`; `loadModel()` now resolves to `ModelLoadRuntimeMetrics | null` instead of `void`.
- `runtime.ts`'s `generate()` now requests `stream_options: {include_usage: true}` and checks `chunk.usage` independently of `chunk.choices[0]` on every iteration (WebLLM's usage trailer has empty `choices`, so it would otherwise be silently skipped by the existing content-handling branch). `buildGenerationTokenUsage()` validates the raw payload (finite non-negative integers for all three counts, `total_tokens === prompt_tokens + completion_tokens`) before trusting any of it; a single malformed field, or no payload at all (an aborted/interrupted/degenerate-output stream never reaches the trailer), degrades the whole measurement to `tokenCountConfidence: "unavailable"`. Once validated, `overallCompletionTokensPerSecond` is derived from `ai-runtime`'s OWN measured FULL inference wall-clock duration (`generationDurationMs` — from immediately before the completion request to full stream consumption, INCLUDING the usage trailer) — deliberately never from WebLLM's self-reported `usage.extra.*` rates, to avoid mixing two different clocks into one figure that could fail a later Phase 0 consistency check. No prompt-throughput field is derived or exposed at all.
- A subsequent review of this same Phase 1 pass (before any of it was committed) found the initial field name `generationTokensPerSecond` semantically ambiguous — its denominator (the full wall-clock span above) includes prefill/TTFT/decode/stream-delivery/usage-trailer-wait, not just decode, so a name suggesting decode-only "generation speed" risked being misread by a future Phase-2 consumer. Renamed to `overallCompletionTokensPerSecond` and documented explicitly as NOT decode-only. The review also found the paired `promptTokensPerSecond` field (`promptTokens / timeToFirstTokenMs`) was not actually exact prefill throughput — TTFT includes worker/message round-trip and first-token sampling, not just prefill — so it could not honestly sit inside an object whose own discriminant claims `"exact"`. Removed entirely rather than kept alongside a weaker confidence tag: Phase 1 does not need prompt throughput, and `promptTokens`/`completionTokens`/`totalTokens` (the genuinely exact counts) are unaffected. The bullets below describe the corrected, final shape — not a separate follow-up sprint, since nothing from the initial pass had shipped yet.
- Metrics are attached at every `{type: "done", ...}` yield site (natural completion/length/tool-call/unknown-terminal, degenerate-output interruption, stream abort, and a thrown-error cancellation) — never at a plain `{type: "error", ...}` chunk (a genuine runtime failure carries no fabricated "done" metrics). `completedAt`/`generationDurationMs` are derived arithmetically from the already-computed `generationStartedAt`/`totalTimeMs` rather than an extra `Date.now()` read, so this phase adds zero new timer reads to the natural-completion path (kept the existing timestamp-mocked test's own "exactly 3 Date.now() calls" invariant intact).
- Extended `@free-ai-open/local-logs`' `LocalLogPerformanceMetrics`/`sanitizePerformanceMetrics()` with `tokenCountConfidence`/`exactGeneratedTokenCount`/`exactPromptTokenCount`/`exactOverallCompletionTokensPerSecond`, each independently validated and dropped (never coerced) when malformed, kept fully separate from the pre-existing heuristic `tokensPerSecond` field. No `exactPromptTokensPerSecond`-style field exists, matching `GenerationTokenUsageExact`'s own omission.
- No new workspace dependency: `ai-runtime`'s `package.json` is unchanged (still `device-profiler`/`local-logs`/`logger`/`@mlc-ai/web-llm` only). A future benchmark-runner phase translates `GenerationRuntimeMetrics` into the Phase 0 `ModelBenchmarkResult` contract, exactly as `apps/web` already translates raw `ai-runtime` chunks into `ModelPerformanceObservation` today.
- Updated `docs/architecture.md` (new "Phase 1: runtime-backed performance metrics" section, plus a precision fix to the pre-existing "Measured-performance integration" section), `docs/privacy.md`, `docs/security.md`, `docs/roadmap.md`, and `CHANGELOG.md`.

### Tests

- `runtime.test.ts`: 23 new tests under a `GenerationRuntimeMetrics` describe block -- exact usage capture/validation (valid payload with no prompt-throughput field present, an explicit proof that `overallCompletionTokensPerSecond` is computed from the FULL wall-clock and not a decode-only surrogate, missing payload, fractional/negative/NaN/Infinite counts, a mismatched total, the zero-token/zero-rate case, and that `stream_options.include_usage` is actually requested); TTFT (recorded from the first raw chunk, not overwritten by later ones, unaffected by an empty/non-content chunk, and unaffected by a simulated slow downstream consumer); lifecycle correctness (length-limited/Stop/degenerate-output/thrown-cancellation all report `"unavailable"` usage honestly; a genuine runtime failure carries no metrics at all; a stale chunk from an abandoned/stalled generation never contributes usage to whatever superseded it; two sequential `generate()` calls -- mirroring two Continue attempts -- keep fully independent, non-aggregated metrics); and a privacy test proving `GenerationRuntimeMetrics`' own top-level shape is a closed allowlist that cannot carry an adversarial prompt's content through, even serialized.
- Fixed 13 pre-existing assertions across `runtime.test.ts` (and the mock fixtures in `apps/web/app/_lib/continuationExecution.test.ts`, `apps/web/app/_runtime/persistentRuntimeLifecycle.test.ts`, `apps/web/app/_runtime/routingOrchestration.test.ts`) that compared a `"done"` chunk by exact equality and would otherwise now fail merely because `metrics` is a new required field unrelated to what those tests actually assert -- added `omitMetrics()`/`omitAllMetrics()` test helpers rather than hardcoding a metrics object into every one of them.
- `local-logs/index.test.ts`: 5 new tests for the extended schema (exact fields round-trip under the renamed field; `"unavailable"` confidence stored distinctly from omitting it; a fractional/negative exact count is dropped, not coerced; an unrecognized confidence value is rejected; an injected `promptTokensPerSecond`/`exactPromptTokensPerSecond`-shaped field never enters the stored record, since neither name is allowlisted).

### Known limitations after Sprint 6.23

- No benchmark runner exists yet -- `GenerationRuntimeMetrics` is produced by every real generation but nothing yet persists it into a `ModelBenchmarkResult`.
- Model load time has no download-vs-initialization split: the installed WebLLM version does not expose one (`InitProgressReport.text` is free-form, not a phase enum), so `ModelLoadRuntimeMetrics.loadTimeMs` remains honestly labeled as "however long this `loadModel()` call took."
- Exact prompt/prefill throughput is not exposed in this phase at all (see the review-correction bullet above) -- reconsidering this later would need its own explicit provenance/confidence model, not a bare number.
- WebLLM's own `usage.extra.*` fields (`prefill_tokens_per_s`, `decode_tokens_per_s`, `e2e_latency_s`, an opt-in `latencyBreakdown`) are known to exist but are not surfaced anywhere in this phase, to avoid mixing two different clocks into one figure.
- Nothing in `apps/web` reads `chunk.metrics`/`loadModel()`'s return value yet (e.g. into `ModelPerformanceObservation`) -- this phase only extends `ai-runtime`'s own output, per its explicit scope.
