...
- Phase 1: benchmark runner — drive `@free-ai-open/ai-runtime` through one bounded "quick"-preset run and produce a real `ModelBenchmarkResult`.
- Phase 2: `/benchmarks` page — consumer-friendly presentation ("Model load time", "Time to first response", "Generation speed", "Stability"), technical fields on demand only.
- Phase 3: router integration — benchmark evidence as additional router input, never a hard-eligibility override, never dominating long-term observation-based stability scoring.
- Phase 4: `standard`/`extended` presets, if still wanted once "quick" has real usage evidence of its own value.
