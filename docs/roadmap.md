...
`v0.7.0-alpha` is being built in phases (see "Adaptive Model Router v1" below); **Phases 0 through 5 are complete**. `AppRuntimeProvider` computes a `RouterDecision` before the first model load and applies it: safe model switching with download consent, and real observation recording from `ai-runtime`. `/settings` now offers automatic (recommended) or manual model selection, and `/chat` shows a one-line plain-language reason for the current pick. Remaining phases are review/testing/release only. Next up:
...
- **v0.7.0-alpha Phase 5 — Router UI and advanced settings: complete.** `/settings` exposes automatic/manual model selection (`ManualModelPicker`, friendly name/size/languages/tasks/device-suitability/cached-status per model, ineligible models shown disabled with the router's own reason), `/chat` shows one plain-language sentence explaining the current pick plus richer status wording (Choosing a local model, Download required, Trying a lighter model), and `/debug`'s adaptive-router panel gained per-model cache status and the real automatic/manual mode.
...
- **Future:** source real `promptTokensPerSecond`/`generationTokensPerSecond` from `ai-runtime`'s `generate()` once it surfaces real WebLLM token counts, and give WebGPU device-loss its own distinct `RuntimeErrorCode` instead of folding it into `out_of_memory`, so the `device_lost` observation outcome becomes reachable.
