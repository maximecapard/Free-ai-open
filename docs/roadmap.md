...
`v0.7.0-alpha` is being built in phases (see "Adaptive Model Router v1" below); **Phase 0 (contracts and architecture), Phase 1A (Capability Profiler v2), and Phase 1B (Model Registry v2) are complete**. Phase 0 added shared contracts and package boundaries. Phase 1A populates the coarse static capability profile locally. Phase 1B adds five strict, source- and license-documented records verified against WebLLM `0.2.84`, with conservative compatibility/language/task metadata and a public browser smoke matrix. The local benchmark, adaptive router core, runtime model-selection integration, and router UI are still future phases. Next up:
