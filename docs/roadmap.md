...
`v0.7.0-alpha` is being built in phases (see "Adaptive Model Router v1" below); **Phase 0 (contracts and architecture) and Phase 1A (Capability Profiler v2) are complete**. Phase 0 added the `StaticCapabilityProfile`/`LocalBenchmarkResult`/`ModelPerformanceObservation`/`RouterInput`/`RouterDecision`/`ModelRegistryRecord` v2 contract types (`@free-ai-open/types`, `@free-ai-open/model-router`, `@free-ai-open/model-registry`), local schema-versioned preference stores, and package-boundary decisions. Phase 1A now populates the coarse static capability profile locally with normalized GPU classes, selected feature/limit buckets, fallback-adapter status, capability class, confidence, expiry, and diagnostic sanitization. The local benchmark, model registry v2 data, adaptive router core, runtime model-selection integration, and router UI are still future phases. Next up:
...
1B. Model Registry v2 — real, verified `ModelRegistryRecord` entries (may run in parallel with Capability Profiler v2 in a separate worktree).
