...
`v0.7.0-alpha` is being built in phases (see "Adaptive Model Router v1" below); **Phases 0 through 3 are complete**. The pure adaptive core can now produce a deterministic `RouterDecision`, but the application does not apply it to WebLLM yet. Runtime integration and router UI remain future phases. Next up:
...
- **Phase 3 complete:** the pure router uses validated French/multilingual metadata when French is selected. Phase 4 still needs to supply the locale and apply the resulting decision to the runtime.
