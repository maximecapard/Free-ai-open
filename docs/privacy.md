...
FreeAI Open's next release will add a user-initiated, local model performance benchmark — distinct from the existing generic device/WebGPU compute microbenchmark above, which stays unchanged. As of this phase, only the local data contract and persistence layer exist (`@free-ai-open/types`' `ModelBenchmarkResult`, `@free-ai-open/model-benchmark`'s local IndexedDB persistence); there is no benchmark runner and no dedicated benchmarks page yet, so nothing described here is reachable from the app today. It is documented now so the privacy guarantees are settled before any runner is built, not discovered afterward.
