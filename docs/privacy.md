...
Generation safety is an alpha safeguard against unstable local model output. If a reply is cancelled, fails, or is detected as degenerate output, FreeAI Open removes the partial assistant response from the chat UI and does not save it as a completed assistant message. As of v0.7.1-alpha, a genuine stall or absolute safety-limit interruption is a deliberate exception when it already produced visible text: that partial reply is saved locally with `status: "incomplete"`, and that provenance survives refresh and local export/import. An interruption that produced no output is still removed. Technical events, local logs, and diagnostic reports remain limited to allowlisted metadata and never receive the generated text or its local conversation record.

Also as of v0.7.1-alpha, a generation that exhausts its output-token budget before reaching a natural stop (WebLLM's own `finish_reason: "length"`), requests a response shape FreeAI Open does not support (`"tool_calls"` — FreeAI Open never requests tool use, so this should not normally occur), or ends its stream without any explicit stop reason at all is tracked as its own explicit outcome — never confused with a stall/safety-limit interruption, and never silently treated as a normal successful reply. Any partial text already produced is preserved and saved with the same durable `status: "incomplete"` provenance described above, plus a specific reason code (a bounded, allowlisted technical value — never response content) that survives refresh and local export/import, so the specific "generation limit reached" notice (for example) is still shown after reloading the page, not only during the session that produced it. Some local models (currently the Qwen3-family registry records) produce an internal `<think>...</think>` reasoning block before their visible answer; if the output budget is exhausted while that block is still open, the chat UI shows a collapsible reasoning panel with an explicit notice that the generation stopped before a final answer, rather than presenting truncated reasoning text as if it were the intended reply. The panel and its notices are presentation-only: they read the already-locally-stored message content and are never sent anywhere, and reasoning content is held to the exact same no-logging guarantee as every other assistant message in this document.

Local storage never silently truncates a saved reply without a signal: a message is only ever cut down to fit a generous storage ceiling (comfortably above what a normal reply plus its allowed manual continuations can produce) in the rare case that ceiling is genuinely reached, and doing so always marks that message `status: "incomplete"` with a specific "storage limit reached" reason rather than quietly saving less than what the user actually saw.

A manual "Continue" action lets the user ask the local model to keep generating from where an interrupted reply left off. The prompt used for this only ever goes to the local model itself; it is not stored as a separate conversation message, not exported, and not logged — only the model's own additional output, appended to the existing local reply, is saved.
...
Conversation content remains local browser data. Completed messages are persisted through `@free-ai-open/conversation-store`; partial assistant output from a stopped, failed, or degenerate generation is removed and not saved. A genuine stall or safety-limit interruption that already produced visible text is saved instead with durable incomplete provenance, rather than presented or exported as a normal completed reply.
...
FreeAI Open's next release will add a user-initiated, local model performance benchmark — distinct from the existing generic device/WebGPU compute microbenchmark above, which stays unchanged. As of Phase 0, only the local data contract and persistence layer exist (`@free-ai-open/types`' `ModelBenchmarkResult`, `@free-ai-open/model-benchmark`'s local IndexedDB persistence). Phase 1 additionally lets `@free-ai-open/ai-runtime` capture real, tokenizer-backed usage counts per generation (see its own bullet below), but there is still no benchmark runner and no dedicated benchmarks page, so nothing described here is reachable from the app today. It is documented now so the privacy guarantees are settled before any runner is built, not discovered afterward.

- **Phase 1's runtime usage capture stays technical-only, with no new network path.** `@free-ai-open/ai-runtime` requests WebLLM's own token-usage trailer (`stream_options: {include_usage: true}`) to capture exact `completion_tokens`/`prompt_tokens`/`total_tokens` — never the prompt or generated text itself, which this request option has no bearing on. The resulting `GenerationRuntimeMetrics` object (timestamps, durations, and the token counts/rate) is attached to `generate()`'s own return value and to `@free-ai-open/local-logs`' technical log fields; it is never sent anywhere, and adds no `fetch`/`sendBeacon`/`WebSocket` call anywhere in `ai-runtime`.

- **Runs locally, stays local.** A benchmark run will execute entirely in the browser through the same client-only `ai-runtime`/Worker boundary every other local generation already uses. No account, no server inference, and no remote benchmark upload — a result is never sent anywhere, ever, by design of the contract itself (there is no upload code path to remove later; none is planned).
- **No benchmark prompt or generated content is ever persisted.** `ModelBenchmarkResult` has no field capable of holding prompt text or generated response text — only bounded technical measurements (load time, time-to-first-token, generation duration, a token count only when a real runtime/tokenizer-backed count is available, and a technical outcome code) and coarse references. This is enforced structurally by the contract's own shape, not merely by convention: `@free-ai-open/model-benchmark`'s `sanitizeModelBenchmarkResult()` rebuilds a brand-new object from an exact allowlist on every read/write, the same discipline every other local store in this document already follows, and every storage backend re-runs this same check on its own write path as defense in depth — the raw backends are not exported from the package's public API, so there is no reachable path that persists a result without going through this sanitizer.
- **No exact device fingerprint.** A result references the device only via the same coarse `capabilityProfileKey` string the existing device benchmark already uses (see "Device capability profiling" above) — never a full capability profile, never a raw GPU adapter string, never a hardware serial or vendor ID beyond the existing coarse allowlisted classes. The key itself is validated against a fixed, low-entropy, four-segment grammar (form factor / capability class / WebGPU availability / adapter kind, each from its own small allowlist) rather than merely a bounded-length string, so an exact GPU description, a UUID, or any other high-entropy value injected into this field can never be persisted even if a caller attempted it.
- **Model/version provenance, never a unique identifier.** A result records which public registry model was benchmarked, the registry version, the model's public quantization/verification metadata, and the WebLLM library version — all already-public, non-identifying technical facts (the same category as "Model registry records" above), used only to detect when a result is stale (the registry or runtime has moved on) and should be ignored.
- **Bounded, expiring, user-clearable local history.** Results are capped locally (100 total, 20 per model — oldest dropped first) and expire after 30 days by default even if nothing about the model/device changed. A user will be able to clear all benchmark history or just one model's history, mirroring the existing "erase conversations"/"erase local logs" controls in "User controls" above (the clearing functions already exist in `@free-ai-open/model-benchmark`; the UI control ships with the runner/benchmarks page, not in this phase).
- **A future diagnostic-report integration** may include a coarse benchmark summary (technical reason codes/metrics only, matching the existing `contentLogged: false` guarantee described throughout this document) — not implemented in this phase.
