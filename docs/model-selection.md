...
Everything above describes the active v0.6 router (`selectRecommendedModel()`), which still makes every routing decision today. `v0.7.0-alpha` is adding a more adaptive router in phases; Phase 0 (contracts and architecture) and Phase 1A (Capability Profiler v2) are complete, without replacing the active router above:
