...
## v0.8.0-alpha — local model benchmark evidence (defined, not wired)

`routeAdaptiveModel()` does not read `ModelBenchmarkResult` yet. `@free-ai-open/types`' `ModelBenchmarkResult` contract and `@free-ai-open/model-benchmark`'s persistence/compatibility/evidence-aggregation layer exist (see `docs/architecture.md`'s "Local model benchmarking" section), but this is Phase 0 (contracts and architecture) of a separate, not-yet-started benchmark runner/router-integration effort. When a future phase does wire this in, benchmark evidence must remain additional evidence alongside — never a replacement for — the real `ModelPerformanceObservation` history above: it must not instantly override a hard eligibility gate, one run must not dominate long-term stability observations built from many real generations (`summarizeModelBenchmarkEvidence()`'s count-gated `evidenceLevel` already makes a single sample "weak" evidence by construction, ahead of any router-scoring weight), and a stale or incompatible result (contract version, registry, WebLLM, capability profile, or browser family mismatch) must be ignored outright rather than down-weighted.
